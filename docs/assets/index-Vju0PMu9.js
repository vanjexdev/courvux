...
`,Ue=!1;function Be(){if(Ue||typeof document>"u")return;Ue=!0;const e=document.createElement("style");e.id="cv-transitions-el",e.textContent=xt,document.head.appendChild(e)}var qe=!1;function wt(){if(qe||typeof document>"u")return;qe=!0;const e=document.createElement("style");e.id="cv-cloak-style",e.textContent="[cv-cloak]{display:none!important}",document.head.appendChild(e)}function kt(e){if(typeof window<"u"&&"Sanitizer"in window){const o=document.createElement("div");return o.setHTML(e,{sanitizer:new window.Sanitizer}),o.innerHTML}const t=new DOMParser().parseFromString(e,"text/html");return t.querySelectorAll("script,iframe,object,embed,form,meta,link,style").forEach(o=>o.remove()),t.querySelectorAll("*").forEach(o=>{Array.from(o.attributes).forEach(c=>{(c.name.startsWith("on")||c.value.trim().toLowerCase().startsWith("javascript:"))&&o.removeAttribute(c.name)})}),t.body.innerHTML}async function oe(e,t,o){var y,S,x,P,C,w,U,T,I,M,O,R,$,d,r,f,k;const c=Array.from(e.childNodes);let l=0;for(;l<c.length;){const A=c[l];if(A.nodeType===3){const i=A.textContent||"",a=i.match(/\{\{([\s\S]+?)\}\}/g);if(a){const s=i,p=()=>{let m=s;a.forEach(u=>{const h=u.replace(/^\{\{\s*/,"").replace(/\s*\}\}$/,"");m=m.replace(u,H(h,t)??"")}),A.textContent=m};a.forEach(m=>{K(m.replace(/^\{\{\s*/,"").replace(/\s*\}\}$/,""),o,p)}),p()}l++;continue}if(A.nodeType!==1){l++;continue}const n=A,g=n.tagName.toLowerCase();if(n.hasAttribute("cv-pre")){n.removeAttribute("cv-pre"),l++;continue}if(n.hasAttribute("cv-once")){n.removeAttribute("cv-once"),await oe(n,t,{...o,subscribe:()=>()=>{},storeSubscribeOverride:()=>()=>{}}),l++;continue}if(n.hasAttribute("cv-cloak")&&n.removeAttribute("cv-cloak"),n.hasAttribute("cv-teleport")){const i=n.getAttribute("cv-teleport");n.removeAttribute("cv-teleport");const a=document.querySelector(i)??document.body,s=document.createComment(`cv-teleport: ${i}`);n.replaceWith(s),await oe(n,t,o),a.appendChild(n),l++;continue}if(n.hasAttribute("cv-memo")){const i=n.getAttribute("cv-memo");n.removeAttribute("cv-memo");const a=()=>i.split(",").map(h=>H(h.trim(),t));let s=a();const p=[],m=h=>(p.push(h),()=>{const v=p.indexOf(h);v>-1&&p.splice(v,1)});await oe(n,t,{...o,subscribe:(h,v)=>m(v),storeSubscribeOverride:(h,v,_)=>m(_)});const u=K(i,o,()=>{const h=a();h.some((v,_)=>v!==s[_])&&(s=h,[...p].forEach(v=>v()))});(y=o.registerCleanup)==null||y.call(o,()=>u()),l++;continue}if(n.hasAttribute("cv-data")){const i=n.getAttribute("cv-data").trim();n.removeAttribute("cv-data");let a={},s={};if(i.startsWith("{")){const p=H(i,t)??{};Object.entries(p).forEach(([m,u])=>{typeof u=="function"?s[m]=u:a[m]=u})}else if(i){const p=(S=o.components)==null?void 0:S[i];if(p){const m=typeof p.data=="function"?p.data():p.data??{};m instanceof Promise||Object.assign(a,m),Object.assign(s,p.methods??{})}}if(o.createChildScope){const p=o.createChildScope(a,s);(x=o.registerCleanup)==null||x.call(o,p.cleanup),await oe(n,p.state,{...o,subscribe:p.subscribe})}else await oe(n,{...t,...a,...s},o);l++;continue}if(n.hasAttribute("cv-for")){const i=n.getAttribute("cv-for");n.removeAttribute("cv-for");const a=i.match(/^\(?(\w+)(?:,\s*(\w+))?\)?\s+in\s+(.+)$/);if(a){const[,s,p,m]=a,u=n.getAttribute(":key")??null;u&&n.removeAttribute(":key");const h=n.getAttribute("cv-transition")??null;h&&n.removeAttribute("cv-transition");const v=document.createComment(`cv-for: ${m}`);n.replaceWith(v);let _=[],D=[];const j=new Map,W=async()=>{var B;const N=H(m,t),F=N?typeof N=="number"?Array.from({length:N},(z,V)=>[V+1,V]):Array.isArray(N)?N.map((z,V)=>[z,V]):Object.entries(N).map(([z,V])=>[V,z]):[];if(u){const z=[],V=new Map,Q=new Set;for(const[Y,X]of F){const ae=H(u,Ne(t,Y,s,X,p));Q.has(ae)&&console.warn(`[courvux] cv-for: duplicate :key "${ae}" in "${m}"`),Q.add(ae),z.push(ae),V.set(ae,[Y,X])}const G=[];for(const[Y,{el:X,destroy:ae}]of j)V.has(Y)||(h?(X.classList.add(`${h}-leave`),G.push(ie(X).then(()=>{var re;X.classList.remove(`${h}-leave`),ae(),(re=X.parentNode)==null||re.removeChild(X),j.delete(Y)}))):(ae(),(B=X.parentNode)==null||B.removeChild(X),j.delete(Y)));G.length&&await Promise.all(G);const q=v.parentNode,te=[];for(const Y of z){const[X,ae]=V.get(Y);if(j.has(Y)){const re=j.get(Y);re.itemRef!==X&&(re.reactive[s]=X,re.itemRef=X),p&&(re.reactive[p]=ae)}else{const re=n.cloneNode(!0),Se=[],{subscribe:ht,createReactiveState:ft}=Pe(),Ee=ft({[s]:X,...p?{[p]:ae}:{}}),vt=new Proxy({},{has(se,ee){return!0},get(se,ee){return typeof ee!="string"?t[ee]:ee===s||p&&ee===p?Ee[ee]:t[ee]},set(se,ee,ce){return ee===s||p&&ee===p?(Ee[ee]=ce,!0):(t[ee]=ce,!0)}}),gt={...o,subscribe:(se,ee)=>{const ce=se.split(".")[0];let de;return ce===s||p&&ce===p?de=ht(ce,ee):de=o.subscribe(se,ee),Se.push(de),de},storeSubscribeOverride:(se,ee,ce)=>{const de=ve(se,ee,ce);return Se.push(de),de}},_e=document.createDocumentFragment();_e.appendChild(re),await oe(_e,vt,gt);const $e=_e.firstChild??re;h&&$e.classList.add(`${h}-enter`),j.set(Y,{el:$e,reactive:Ee,itemRef:X,destroy:()=>Se.forEach(se=>se())}),h&&te.push($e)}}let J=v.nextSibling,pe=0;for(const Y of z){const{el:X}=j.get(Y);X!==J?pe++:J=X.nextSibling}if(pe>0)if(pe>z.length>>1){const Y=document.createDocumentFragment();for(const X of z)Y.appendChild(j.get(X).el);q.insertBefore(Y,v.nextSibling)}else{J=v.nextSibling;for(const Y of z){const{el:X}=j.get(Y);X!==J?q.insertBefore(X,J):J=X.nextSibling}}_=z.map(Y=>j.get(Y).el),te.length&&Promise.all(te.map(Y=>ie(Y).then(()=>Y.classList.remove(`${h}-enter`))))}else{if(D.forEach(G=>G()),D=[],_.forEach(G=>{var q;return(q=G.parentNode)==null?void 0:q.removeChild(G)}),_=[],!F.length)return;const z=v.parentNode,V=v.nextSibling,Q={...o,subscribe:(G,q)=>{const te=o.subscribe(G,q);return D.push(te),te},storeSubscribeOverride:(G,q,te)=>{const J=ve(G,q,te);return D.push(J),J}};for(const[G,q]of F){const te=n.cloneNode(!0),J=document.createDocumentFragment();J.appendChild(te),await oe(J,Ne(t,G,s,q,p),Q);const pe=J.firstChild??te;z.insertBefore(J,V),_.push(pe)}}};(P=o.registerCleanup)==null||P.call(o,()=>{j.forEach(({el:N,destroy:F})=>{var B;F(),(B=N.parentNode)==null||B.removeChild(N)}),j.clear(),D.forEach(N=>N()),_.forEach(N=>{var F;return(F=N.parentNode)==null?void 0:F.removeChild(N)}),_=[]}),K(m,o,W),await W()}l++;continue}if(n.hasAttribute("cv-if")){const i=[],a=n.getAttribute("cv-if");n.removeAttribute("cv-if");const s=document.createComment("cv-if");n.replaceWith(s),i.push({condition:a,template:n,anchor:s});let p=l+1;for(;p<c.length;){const _=c[p];if(_.nodeType===3&&(((C=_.textContent)==null?void 0:C.trim())??"")===""){p++;continue}if(_.nodeType!==1)break;const D=_;if(D.hasAttribute("cv-else-if")){const j=D.getAttribute("cv-else-if");D.removeAttribute("cv-else-if");const W=document.createComment("cv-else-if");D.replaceWith(W),i.push({condition:j,template:D,anchor:W}),p++;continue}if(D.hasAttribute("cv-else")){D.removeAttribute("cv-else");const j=document.createComment("cv-else");D.replaceWith(j),i.push({condition:null,template:D,anchor:j}),p++;break}break}l=p;let m=null,u=!1,h=!1;const v=async()=>{var _,D;if(u){h=!0;return}u=!0;try{do{h=!1,m&&((_=m.parentNode)==null||_.removeChild(m),m=null);for(const j of i)if(j.condition===null||H(j.condition,t)){const W=j.template.cloneNode(!0),N=document.createDocumentFragment();N.appendChild(W),await oe(N,t,o);const F=N.firstChild??W;(D=j.anchor.parentNode)==null||D.insertBefore(N,j.anchor.nextSibling),m=F;break}}while(h)}finally{u=!1}};i.filter(_=>_.condition).forEach(_=>{K(_.condition,o,v)}),await v();continue}if(n.hasAttribute("cv-show")){const i=n.getAttribute("cv-show");n.removeAttribute("cv-show");const a=Array.from(n.attributes).filter(s=>s.name==="cv-transition"||s.name.startsWith("cv-transition:")||s.name.startsWith("cv-transition."));if(a.length>0){const s=F=>(n.getAttribute(F)??"").split(" ").filter(Boolean),p=s("cv-transition:enter"),m=s("cv-transition:enter-start"),u=s("cv-transition:enter-end"),h=s("cv-transition:leave"),v=s("cv-transition:leave-start"),_=s("cv-transition:leave-end"),D=n.getAttribute("cv-transition")??"",j=new Set(D.split(".").slice(1)),W=[...j].find(F=>/^\d+$/.test(F)),N=W?parseInt(W):200;if(p.length||m.length||h.length||v.length){a.forEach(G=>n.removeAttribute(G.name));const F=()=>new Promise(G=>requestAnimationFrame(()=>requestAnimationFrame(()=>G())));let B=!!H(i,t),z=!1,V=null;const Q=async G=>{if(z){V=G;return}z=!0;try{G?(n.style.display="",n.classList.add(...p,...m),await F(),n.classList.remove(...m),n.classList.add(...u),await ie(n),n.classList.remove(...p,...u)):(n.classList.add(...h,...v),await F(),n.classList.remove(...v),n.classList.add(..._),await ie(n),n.classList.remove(...h,..._),n.style.display="none"),B=G}finally{if(z=!1,V!==null&&V!==B){const q=V;V=null,Q(q)}else V=null}};B||(n.style.display="none"),K(i,o,()=>{const G=!!H(i,t);G!==B&&Q(G)})}else{const F=[...j].find(q=>q==="scale"||/^scale$/.test(q)),B=(()=>{const q=[...j].find(te=>/^\d+$/.test(te)&&te!==W);return q?parseInt(q)/100:.9})(),z=[];(!j.has("scale")||j.has("opacity"))&&z.push(`opacity ${N}ms ease`),F&&z.push(`transform ${N}ms ease`),z.length||z.push(`opacity ${N}ms ease`),n.style.transition=(n.style.transition?n.style.transition+", ":"")+z.join(", "),a.forEach(q=>n.removeAttribute(q.name));let V=!!H(i,t);const Q=()=>new Promise(q=>requestAnimationFrame(()=>requestAnimationFrame(()=>q()))),G=async q=>{q?(n.style.display="",n.style.opacity="0",F&&(n.style.transform=`scale(${B})`),await Q(),n.style.opacity="",F&&(n.style.transform=""),await ie(n)):(n.style.opacity="0",F&&(n.style.transform=`scale(${B})`),await ie(n),n.style.display="none",n.style.opacity="",F&&(n.style.transform="")),V=q};V||(n.style.display="none"),K(i,o,()=>{const q=!!H(i,t);q!==V&&G(q)})}}else{const s=n.getAttribute("cv-show-transition"),p=n.getAttribute(":transition");s&&n.removeAttribute("cv-show-transition"),p&&n.removeAttribute(":transition");const m=s??(p?String(H(p,t)):null);if(m){Be();let u=!!H(i,t);u||(n.style.display="none");let h=!1,v=null;const _=async D=>{if(h){v=D;return}h=!0;try{D?(n.style.display="",n.classList.add(`${m}-enter`),await ie(n),n.classList.remove(`${m}-enter`)):(n.classList.add(`${m}-leave`),await ie(n),n.classList.remove(`${m}-leave`),n.style.display="none"),u=D}finally{if(h=!1,v!==null&&v!==u){const j=v;v=null,_(j)}else v=null}};K(i,o,()=>{const D=!!H(i,t);D!==u&&_(D)})}else{const u=()=>{n.style.display=H(i,t)?"":"none"};K(i,o,u),u()}}}if(n.hasAttribute("cv-focus")){const i=n.getAttribute("cv-focus")??"";if(n.removeAttribute("cv-focus"),!i)Promise.resolve().then(()=>n.focus());else{const a=()=>{H(i,t)&&Promise.resolve().then(()=>n.focus())};K(i,o,a),a()}}{const i=Array.from(n.attributes).filter(a=>a.name==="cv-intersect"||a.name.startsWith("cv-intersect:")||a.name.startsWith("cv-intersect."));if(i.length&&typeof IntersectionObserver<"u"){const a=i.find(F=>F.name==="cv-intersect"||F.name==="cv-intersect:enter"||F.name.startsWith("cv-intersect.")),s=i.find(F=>F.name==="cv-intersect:leave"),p=(a==null?void 0:a.value)??"",m=(s==null?void 0:s.value)??"",u=((a==null?void 0:a.name)??"cv-intersect").split("."),h=new Set(u.slice(1)),v=h.has("once");let _=0;if(h.has("half"))_=.5;else if(h.has("full"))_=1;else{const F=[...h].find(B=>B.startsWith("threshold-"));F&&(_=parseInt(F.replace("threshold-",""))/100)}const D=[...h].find(F=>F.startsWith("margin-")),j=D?`${D.replace("margin-","")}px`:void 0;i.forEach(F=>n.removeAttribute(F.name));const W=F=>{if(F)try{new Function("$data",`with($data){${F}}`)(t)}catch(B){console.warn(`[courvux] cv-intersect error "${F}":`,B)}},N=new IntersectionObserver(F=>{F.forEach(B=>{B.isIntersecting?(W(p),v&&N.disconnect()):W(m)})},{threshold:_,...j?{rootMargin:j}:{}});N.observe(n),(w=o.registerCleanup)==null||w.call(o,()=>N.disconnect())}}{const i=Array.from(n.attributes).find(a=>a.name==="cv-html"||a.name.startsWith("cv-html."));if(i){const a=i.value;n.removeAttribute(i.name);const s=i.name.split(".").slice(1).includes("sanitize"),p=()=>{const m=String(H(a,t)??"");n.innerHTML=s?kt(m):m};K(a,o,p),p(),l++;continue}}if(n.hasAttribute("cv-ref")&&!((U=o.components)!=null&&U[g])){const i=n.getAttribute("cv-ref");n.removeAttribute("cv-ref"),o.refs&&(o.refs[i]=n)}const b=!!((T=o.components)!=null&&T[g]),E=Array.from(n.attributes).find(i=>i.name==="cv-model"||i.name.startsWith("cv-model."));if(E&&!b){const i=E.value;n.removeAttribute(E.name);const a=new Set(E.name.split(".").slice(1)),s=n,p=(I=s.type)==null?void 0:I.toLowerCase(),m=u=>{if(a.has("number")){const h=parseFloat(u);return isNaN(h)?u:h}return a.has("trim")?u.trim():u};if(p==="checkbox"){const u=()=>{const h=H(i,t);s.checked=Array.isArray(h)?h.includes(s.value):!!h};ue(i,o,u),u(),s.addEventListener("change",()=>{const h=H(i,t);if(Array.isArray(h)){const v=[...h];if(s.checked)v.includes(s.value)||v.push(s.value);else{const _=v.indexOf(s.value);_>-1&&v.splice(_,1)}le(i,t,v)}else le(i,t,s.checked)})}else if(p==="radio"){const u=()=>{s.checked=H(i,t)===s.value};ue(i,o,u),u(),s.addEventListener("change",()=>{s.checked&&le(i,t,m(s.value))})}else if(n.hasAttribute("contenteditable")){const u=n,h=()=>{const v=String(H(i,t)??"");u.innerText!==v&&(u.innerText=v)};if(ue(i,o,h),h(),a.has("debounce")){const v=[...a].find(j=>/^\d+$/.test(j)),_=v?parseInt(v):300;let D;u.addEventListener("input",()=>{clearTimeout(D),D=setTimeout(()=>le(i,t,m(u.innerText)),_)})}else{const v=a.has("lazy")?"blur":"input";u.addEventListener(v,()=>le(i,t,m(u.innerText)))}}else{const u=()=>{s.value=H(i,t)??""};if(ue(i,o,u),u(),a.has("debounce")){const h=[...a].find(D=>/^\d+$/.test(D)),v=h?parseInt(h):300;let _;s.addEventListener("input",()=>{clearTimeout(_),_=setTimeout(()=>le(i,t,m(s.value)),v)})}else{const h=g==="select"||a.has("lazy")?"change":"input";s.addEventListener(h,()=>le(i,t,m(s.value)))}}}if(o.directives&&Array.from(n.attributes).forEach(i=>{var W,N;if(!i.name.startsWith("cv-"))return;const a=i.name.slice(3).split("."),s=a[0],p=a.slice(1),m=s.indexOf(":"),u=m>=0?s.slice(0,m):s,h=m>=0?s.slice(m+1):void 0,v=o.directives[u];if(!v)return;const _=i.value;n.removeAttribute(i.name);const D=typeof v=="function"?{onMount:v}:v,j={value:_?H(_,t):void 0,arg:h,modifiers:Object.fromEntries(p.map(F=>[F,!0]))};(W=D.onMount)==null||W.call(D,n,j),D.onUpdate&&_&&K(_,o,()=>{j.value=H(_,t),D.onUpdate(n,j)}),D.onDestroy&&((N=o.registerCleanup)==null||N.call(o,()=>D.onDestroy(n,j)))}),g==="slot"){const i=n.getAttribute("name")??"default",a=(M=o.slots)==null?void 0:M[i];if(a){const s={};Array.from(n.attributes).forEach(u=>{u.name.startsWith(":")&&(s[u.name.slice(1)]=H(u.value,t))});const p=await a(s),m=document.createDocumentFragment();p.forEach(u=>m.appendChild(u)),n.replaceWith(m)}else{const s=document.createDocumentFragment();for(;n.firstChild;)s.appendChild(n.firstChild);await oe(s,t,o),n.replaceWith(s)}l++;continue}if(g==="cv-transition"){Be();const i=n.getAttribute("name")??"fade",a=n.getAttribute(":show")??null;n.removeAttribute("name"),a&&n.removeAttribute(":show");const s=document.createElement("div");for(s.className="cv-t-wrap";n.firstChild;)s.appendChild(n.firstChild);if(n.replaceWith(s),await oe(s,t,o),a){let p=!!H(a,t),m=!1,u=null;p||(s.style.display="none");const h=async v=>{if(m){u=v;return}m=!0;try{v?(s.style.display="",s.classList.add(`${i}-enter`),await ie(s),s.classList.remove(`${i}-enter`)):(s.classList.add(`${i}-leave`),await ie(s),s.classList.remove(`${i}-leave`),s.style.display="none"),p=v}finally{if(m=!1,u!==null&&u!==p){const _=u;u=null,h(_)}else u=null}};K(a,o,()=>{const v=!!H(a,t);v!==p&&h(v)})}l++;continue}if(g==="router-view"&&o.mountRouterView){const i=n.getAttribute("name")??void 0;n.setAttribute("aria-live","polite"),n.setAttribute("aria-atomic","true"),await o.mountRouterView(n,i),l++;continue}if(g==="router-link"){const i=n.getAttribute(":to"),a=n.getAttribute("to"),s=()=>i?String(H(i,t)??"/"):a||"/",p=F=>String(F).replace(/[&"<>]/g,B=>({"&":"&amp;",'"':"&quot;","<":"&lt;",">":"&gt;"})[B]);let m="";Array.from(n.attributes).forEach(F=>{F.name==="to"||F.name===":to"||(m+=` ${F.name}="${p(F.value)}"`)});const u=document.createElement("div");u.innerHTML=`<a${m}></a>`;const h=u.firstElementChild;for(;n.firstChild;)h.appendChild(n.firstChild);const v=((O=o.router)==null?void 0:O.base)??"",_=F=>v?F===v?"/":F.startsWith(v+"/")?F.slice(v.length)||"/":F||"/":F||"/",D=()=>{var F;return((F=o.router)==null?void 0:F.mode)==="history"?_(window.location.pathname):window.location.hash.slice(1)||"/"},j=()=>{var z;const F=s(),B=D()===F;((z=o.router)==null?void 0:z.mode)==="history"?h.href=`${v}${F}`:h.href=`#${F}`,B?(h.setAttribute("aria-current","page"),h.classList.add("active")):(h.removeAttribute("aria-current"),h.classList.remove("active"))};((R=o.router)==null?void 0:R.mode)==="history"?(h.addEventListener("click",F=>{F.preventDefault(),o.router.navigate(s())}),window.addEventListener("popstate",j)):window.addEventListener("hashchange",j),i&&ue(i,o,j),j();const W=document.createDocumentFragment();W.appendChild(h),await oe(W,t,o);const N=W.firstChild??h;n.replaceWith(N),l++;continue}if(g==="component"&&n.hasAttribute(":is")&&o.mountDynamic){const i=n.getAttribute(":is");n.removeAttribute(":is");const a=document.createComment("component:is");n.replaceWith(a),await o.mountDynamic(a,i,n,t,o),l++;continue}if(($=o.components)!=null&&$[g]&&o.mountElement){await o.mountElement(n,g,t,o),l++;continue}{const i=Array.from(n.attributes).find(a=>a.name==="cv-intersect"||a.name.startsWith("cv-intersect."));if(i&&typeof IntersectionObserver<"u"){const a=new Set(i.name.split(".").slice(1));n.removeAttribute(i.name);const s=H(i.value,t);let p,m=0,u="0px",h=a.has("once");if(typeof s=="function"?p=v=>s.call(t,v):s&&typeof s=="object"&&(typeof s.handler=="function"&&(p=v=>s.handler.call(t,v)),s.threshold!==void 0&&(m=s.threshold),s.margin&&(u=s.margin),s.once&&(h=!0)),p){const v=new IntersectionObserver(_=>{const D=_[0];p(D),h&&D.isIntersecting&&v.disconnect()},{threshold:m,rootMargin:u});v.observe(n),(d=o.registerCleanup)==null||d.call(o,()=>v.disconnect())}}}if(n.hasAttribute("cv-resize")){const i=n.getAttribute("cv-resize");if(n.removeAttribute("cv-resize"),typeof ResizeObserver<"u"){const a=H(i,t);let s,p="content-box";if(typeof a=="function"?s=m=>a.call(t,m):a&&typeof a=="object"&&(typeof a.handler=="function"&&(s=m=>a.handler.call(t,m)),a.box&&(p=a.box)),s){const m=new ResizeObserver(u=>{u[0]&&s(u[0])});m.observe(n,{box:p}),(r=o.registerCleanup)==null||r.call(o,()=>m.disconnect())}}}if(n.hasAttribute("cv-scroll")){const i=n.getAttribute("cv-scroll");n.removeAttribute("cv-scroll");const a=H(i,t);let s,p=0;if(typeof a=="function"?s=m=>a.call(t,m):a&&typeof a=="object"&&(typeof a.handler=="function"&&(s=m=>a.handler.call(t,m)),a.throttle&&(p=a.throttle)),s){let m=0;const u=()=>{const h=Date.now();p>0&&h-m<p||(m=h,s({scrollTop:n.scrollTop,scrollLeft:n.scrollLeft,scrollHeight:n.scrollHeight,scrollWidth:n.scrollWidth,clientHeight:n.clientHeight,clientWidth:n.clientWidth}))};n.addEventListener("scroll",u,{passive:!0}),(f=o.registerCleanup)==null||f.call(o,()=>n.removeEventListener("scroll",u))}}if(n.hasAttribute("cv-clickoutside")){const i=n.getAttribute("cv-clickoutside");n.removeAttribute("cv-clickoutside");const a=s=>{n.contains(s.target)||(typeof t[i]=="function"?t[i].call(t,s):We(i,t,s))};document.addEventListener("click",a,!0),(k=o.registerCleanup)==null||k.call(o,()=>document.removeEventListener("click",a,!0))}if(n.hasAttribute("cv-bind")){const i=n.getAttribute("cv-bind");n.removeAttribute("cv-bind");const a=n.getAttribute("class")??"",s=n.getAttribute("style")??"";let p=[];const m=()=>{const u=H(i,t)??{},h=Object.keys(u);for(const v of p)v in u||(v==="class"?n.className=a:v==="style"?n.style.cssText=s:n.removeAttribute(v));for(const[v,_]of Object.entries(u))v==="class"?n.className=[a,De(_)].filter(Boolean).join(" "):v==="style"?He(n,_,s):_==null||_===!1?n.removeAttribute(v):n.setAttribute(v,_===!0?"":String(_));p=h};K(i,o,m),m()}const L={enter:"Enter",esc:"Escape",escape:"Escape",space:" ",tab:"Tab",delete:"Delete",backspace:"Backspace",up:"ArrowUp",down:"ArrowDown",left:"ArrowLeft",right:"ArrowRight"};Array.from(n.attributes).forEach(i=>{if(i.name.startsWith("@")||i.name.startsWith("cv:on:")){const a=(i.name.startsWith("@")?i.name.substring(1):i.name.substring(6)).split("."),s=a[0],p=new Set(a.slice(1)),m=[...p].find(_=>_ in L),u=i.value,h=_=>{p.has("prevent")&&_.preventDefault(),p.has("stop")&&_.stopPropagation(),!(p.has("self")&&_.target!==_.currentTarget)&&(m&&_.key!==L[m]||(typeof t[u]=="function"?t[u].call(t,_):We(u,t,_)))},v={};p.has("once")&&(v.once=!0),p.has("passive")&&(v.passive=!0),p.has("capture")&&(v.capture=!0),n.addEventListener(s,h,Object.keys(v).length?v:void 0)}else if(i.name.startsWith(":")){const a=i.name.slice(1),s=i.value;if(a==="class"){const p=n.getAttribute("class")??"",m=()=>{n.className=[p,De(H(s,t))].filter(Boolean).join(" ")};K(s,o,m),m()}else if(a==="style"){const p=n.getAttribute("style")??"",m=()=>He(n,H(s,t),p);K(s,o,m),m()}else if(a.includes("-")){const p=()=>{const m=H(s,t);m==null||m===!1?n.removeAttribute(a):n.setAttribute(a,m===!0?"":String(m))};K(s,o,p),p()}else{const p=()=>{n[a]=H(s,t)??""};K(s,o,p),p()}}}),A.hasChildNodes()&&await oe(A,t,o),l++}}var At=`
...
`;function St(){if(document.getElementById("cv-transitions"))return;const e=document.createElement("style");e.id="cv-transitions",e.textContent=At,document.head.appendChild(e)}async function ge(e,t,o){e.classList.add(`${t}-${o}`);const c=getComputedStyle(e),l=Math.max(parseFloat(c.animationDuration)||0,parseFloat(c.transitionDuration)||0)*1e3;l>0&&await new Promise(y=>{const S=()=>y();e.addEventListener("animationend",S,{once:!0}),e.addEventListener("transitionend",S,{once:!0}),setTimeout(S,l+50)}),e.classList.remove(`${t}-${o}`)}var we=new Map;async function Et(e){if(typeof e!="function")return e;if(we.has(e))return we.get(e);const t=await e();return we.set(e,t.default),t.default}function Ve(e,t){if(e.components)return e.components[t];if(t==="default")return e.component}function Ge(e,t){if(e==="*")return{};const o=[],c=e.replace(/:(\w+)/g,(y,S)=>(o.push(S),"([^/]+)")),l=t.match(new RegExp(`^${c}$`));return l?Object.fromEntries(o.map((y,S)=>[y,l[S+1]])):null}function _t(e,t){if(e==="/")return{params:{},remaining:t};const o=[],c=e.replace(/:(\w+)/g,(y,S)=>(o.push(S),"([^/]+)")),l=t.match(new RegExp(`^${c}(/.+)?$`));return l?{params:Object.fromEntries(o.map((y,S)=>[y,l[S+1]])),remaining:l[o.length+1]||"/"}:null}function ut(e,t=""){return e.map(o=>{var l;if(o.path==="*")return o;const c=((t.endsWith("/")?t.slice(0,-1):t)+o.path).replace(/\/+/g,"/")||"/";return(l=o.children)!=null&&l.length?{...o,path:c,children:ut(o.children,c==="/"?"":c)}:{...o,path:c}})}var me=(e,t)=>new Promise(o=>e(t,o)),Xe=(e,t)=>e!=null&&e.beforeLeave?new Promise(o=>e.beforeLeave(t,o)):Promise.resolve(void 0);function $t(e,t={}){const o=t.mode??"hash",c=Tt(t.base??"");return{routes:ut(e),mode:o,base:c,transition:t.transition,beforeEach:t.beforeEach,afterEach:t.afterEach,scrollBehavior:t.scrollBehavior,navigate(l,y){const S=Ye(l,y==null?void 0:y.query);o==="history"?(history.pushState({},"",`${c}${S}`),window.dispatchEvent(new PopStateEvent("popstate"))):window.location.hash=S},replace(l,y){const S=Ye(l,y==null?void 0:y.query);if(o==="history")history.replaceState({},"",`${c}${S}`),window.dispatchEvent(new PopStateEvent("popstate"));else{const x=window.location.href.split("#")[0];window.location.replace(`${x}#${S}`)}},back(){history.back()},forward(){history.forward()}}}function Tt(e){if(!e||e==="/")return"";let t=e.startsWith("/")?e:`/${e}`;return t.endsWith("/")&&(t=t.slice(0,-1)),t}function Ct(e,t){return t?e===t?"/":e.startsWith(t+"/")?e.slice(t.length)||"/":e||"/":e||"/"}function Ye(e,t){return!t||!Object.keys(t).length?e:`${e}?${new URLSearchParams(t).toString()}`}function Ze(e){if(!e)return{};const t=new URLSearchParams(e.startsWith("?")?e.slice(1):e),o={};return t.forEach((c,l)=>{o[l]=c}),o}function Je(e,t,o,c="default",l){const y=t.base??"",S=()=>t.mode==="history"?Ct(window.location.pathname,y):(window.location.hash.slice(1)||"/").split("?")[0]||"/",x=()=>{if(t.mode==="history")return Ze(window.location.search);const r=window.location.hash.slice(1)||"/",f=r.indexOf("?");return f>=0?Ze(r.slice(f+1)):{}};t.transition&&St();let P=null,C=null,w=null,U=null,T=!1;const I=()=>{T||(T=!0,l==null||l())},M=new Map,O=r=>{var f;if(r!=null&&r.keepAlive&&w){(f=w.deactivate)==null||f.call(w);const k=document.createDocumentFragment();for(;e.firstChild;)k.appendChild(e.firstChild);M.set(P.path,{fragment:k,activation:w}),w=null}else w==null||w.destroy(),w=null,e.innerHTML=""},R=async(r,f,k,A,n)=>{const g=typeof f=="function"&&!we.has(f),b=g?f.__asyncOptions:void 0,E=r.loadingTemplate??(b==null?void 0:b.loadingTemplate);g&&E&&(e.innerHTML=E);let L;try{L=await Et(f)}catch(i){const a=b==null?void 0:b.errorTemplate;if(a)return e.innerHTML=a,{destroy:()=>{e.innerHTML=""}};throw i}return g&&E&&(e.innerHTML=""),o(e,L,k,A,n)},$=async()=>{var k,A,n,g,b,E,L,i;const r=S(),f=x();for(const a of t.routes){if((k=a.children)!=null&&k.length){const p=_t(a.path,r);if(p!==null)for(const m of a.children){const u=Ge(m.path,r);if(u!==null){const h={params:p.params,query:f,path:r,meta:a.meta};if(m.redirect){const j={params:u,query:f,path:r,meta:m.meta},W=typeof m.redirect=="function"?m.redirect(j):m.redirect;t.navigate(W);return}if(t.beforeEach){const j=await me(t.beforeEach,h);if(j){t.navigate(j);return}}if(a.beforeEnter){const j=await me(a.beforeEnter,h);if(j){t.navigate(j);return}}if(m.beforeEnter){const j={params:u,query:f,path:r,meta:m.meta},W=await me(m.beforeEnter,j);if(W){t.navigate(W);return}}const v=`${a.path}::${JSON.stringify(p.params)}`;if(U!==v){const j=await Xe(w,h);if(j){t.navigate(j);return}const W=a.transition??t.transition;W&&e.hasChildNodes()&&await ge(e,W,"leave"),O(C);const N=Ve(a,c);if(N){const F={routes:a.children,mode:t.mode,base:t.base,transition:a.transition??t.transition,beforeEach:t.beforeEach,afterEach:t.afterEach,scrollBehavior:t.scrollBehavior,navigate:(B,z)=>t.navigate(B,z),replace:(B,z)=>t.replace(B,z),back:()=>t.back(),forward:()=>t.forward()};w=await R(a,N,h,c==="default"?a.layout:void 0,c==="default"?F:void 0),(A=w.enter)==null||A.call(w,P)}else e.innerHTML="";U=v,W&&await ge(e,W,"enter")}const _={params:{...p.params,...u},query:f,path:r,meta:m.meta??a.meta};(n=t.afterEach)==null||n.call(t,_,P);const D=(g=t.scrollBehavior)==null?void 0:g.call(t,_,P);D&&window.scrollTo(D.x??0,D.y??0),P=_,C=a,I();return}}}const s=Ge(a.path,r);if(s!==null){U=null;const p={params:s,query:f,path:r,meta:a.meta};if(a.redirect){const _=typeof a.redirect=="function"?a.redirect(p):a.redirect;t.navigate(_);return}if(t.beforeEach){const _=await me(t.beforeEach,p);if(_){t.navigate(_);return}}if(a.beforeEnter){const _=await me(a.beforeEnter,p);if(_){t.navigate(_);return}}const m=await Xe(w,p);if(m){t.navigate(m);return}const u=a.transition??t.transition;u&&e.hasChildNodes()&&await ge(e,u,"leave"),O(C);const h=Ve(a,c);if(h){const _=p.path;if(a.keepAlive&&M.has(_)){const D=M.get(_);e.appendChild(D.fragment),w=D.activation,(b=w.activate)==null||b.call(w),M.delete(_)}else{const D=P;w=await R(a,h,p,c==="default"?a.layout:void 0),(E=w.enter)==null||E.call(w,D)}}else e.innerHTML="",w=null;u&&await ge(e,u,"enter"),(L=t.afterEach)==null||L.call(t,p,P);const v=(i=t.scrollBehavior)==null?void 0:i.call(t,p,P);v&&window.scrollTo(v.x??0,v.y??0),P=p,C=a,I();return}}U=null,O(C),C=null,I()},d=t.mode==="history"?"popstate":"hashchange";return window.addEventListener(d,$),$(),()=>{window.removeEventListener(d,$),w==null||w.destroy(),w=null,M.forEach(({activation:r})=>r.destroy()),M.clear()}}function Ft(){if(typeof window>"u")return null;if(window.__COURVUX_DEVTOOLS__)return window.__COURVUX_DEVTOOLS__;const e=new Map,t={instances:[],stores:[],on(o,c){return e.has(o)||e.set(o,new Set),e.get(o).add(c),()=>{var l;return(l=e.get(o))==null?void 0:l.delete(c)}},_emit(o,c){var l;(l=e.get(o))==null||l.forEach(y=>{try{y(c)}catch{}})},_registerInstance(o){this.instances.push(o),this._emit("mount",o)},_unregisterInstance(o){const c=this.instances.findIndex(l=>l.id===o);if(c!==-1){const l=this.instances[c];this.instances.splice(c,1),this._emit("destroy",l)}},_registerStore(o){this.stores.push(o),o.subscribe(()=>this._emit("store-update",o))}};return window.__COURVUX_DEVTOOLS__=t,t}var Ot=0;function jt(){return++Ot}var Lt=`
...
`;function Pt(){if(document.getElementById("cvd-styles"))return;const e=document.createElement("style");e.id="cvd-styles",e.textContent=Lt,document.head.appendChild(e)}function be(e){return e===null?"null":e===void 0?"undefined":typeof e=="string"?`"${e}"`:typeof e=="object"?JSON.stringify(e):String(e)}function Ke(e){try{return JSON.parse(e)}catch{return e}}function Dt(e){if(typeof document>"u")return;Pt();const t=document.createElement("div");t.id="cvd",document.body.appendChild(t);let o=!1,c="components",l=new Set;const y=document.createElement("div");y.id="cvd-badge",y.innerHTML='<span class="cvd-badge-dot"></span>COURVUX',t.appendChild(y);const S=document.createElement("div");S.id="cvd-panel",S.style.display="none",t.appendChild(S),S.innerHTML=`
        ...
        <div id="cvd-body"></div>
    `;const x=S.querySelector("#cvd-body");y.addEventListener("click",()=>{o=!0,y.style.display="none",S.style.display="flex",U()}),S.querySelector("#cvd-close").addEventListener("click",()=>{o=!1,S.style.display="none",y.style.display=""}),S.querySelectorAll(".cvd-tab").forEach(T=>{T.addEventListener("click",()=>{c=T.dataset.tab,S.querySelectorAll(".cvd-tab").forEach(I=>I.classList.remove("active")),T.classList.add("active"),U()})});const P=S.querySelector("#cvd-head");P.addEventListener("pointerdown",T=>{if(T.target.closest("button"))return;P.setPointerCapture(T.pointerId);const I=T.clientX,M=T.clientY,O=t.offsetLeft,R=t.offsetTop,$=r=>{t.style.right="auto",t.style.bottom="auto",t.style.left=`${O+(r.clientX-I)}px`,t.style.top=`${R+(r.clientY-M)}px`},d=r=>{P.releasePointerCapture(r.pointerId),P.removeEventListener("pointermove",$),P.removeEventListener("pointerup",d),P.removeEventListener("pointercancel",d)};P.addEventListener("pointermove",$),P.addEventListener("pointerup",d),P.addEventListener("pointercancel",d)});function C(){const T=e.instances;if(!T.length){x.innerHTML='<div class="cvd-empty">No hay componentes montados</div>';return}x.innerHTML=T.map(I=>{const M=I.getState(),O=Object.keys(M);return`
            ...
            `}).join(""),x.querySelectorAll(".cvd-inst-head").forEach(I=>{I.addEventListener("click",()=>{const M=I.closest(".cvd-inst"),O=parseInt(M.dataset.id);l.has(O)?l.delete(O):l.add(O),M.classList.toggle("open")})}),x.querySelectorAll(".cvd-val").forEach(I=>{I.addEventListener("click",M=>{M.stopPropagation();const O=I;if(O.querySelector("input"))return;const R=parseInt(O.dataset.inst),$=O.dataset.key,d=e.instances.find(A=>A.id===R);if(!d)return;const r=be(d.getState()[$]);O.classList.add("editing"),O.innerHTML=`<input class="cvd-edit" value='${r.replace(/'/g,"&#39;")}'>`;const f=O.querySelector("input");f.focus(),f.select();const k=()=>{d.setState($,Ke(f.value)),O.classList.remove("editing")};f.addEventListener("blur",k),f.addEventListener("keydown",A=>{A.key==="Enter"&&(A.preventDefault(),k()),A.key==="Escape"&&(O.classList.remove("editing"),U())})})})}function w(){if(!e.stores.length){x.innerHTML='<div class="cvd-empty">No hay store registrado</div>';return}x.innerHTML=e.stores.map((T,I)=>{const M=T.getState(),O=Object.keys(M);return`
...
pnpm add github:vanjexdev/courvux#v0.4.4
...
pnpm add github:vanjexdev/courvux#v0.4.4`,s2:`<script type="importmap">
{
  "imports": {
...
{
  "imports": {
    "courvux": "https://cdn.jsdelivr.net/gh/vanjexdev/courvux@v0.4.4/dist/index.js"
  }
}
<\/script>
