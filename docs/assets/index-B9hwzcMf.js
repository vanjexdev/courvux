...
`)}const io={data:{lang:"js",code:"",label:"",copied:!1},template:`
    ...
    `,exprs:{"label || langLabel":(e=>e.label||e.langLabel),"copied ? '✓ Copied' : 'Copy'":(e=>e.copied?"â Copied":"Copy"),"copy()":(e=>e.copy()),"'language-' + lang":(e=>"language-"+e.lang)},computed:{langLabel(){return so[this.lang]||this.lang.toUpperCase()}},methods:{copy(){var e;(e=navigator.clipboard)==null||e.writeText(this._cleanCode).then(()=>{this.copied=!0,setTimeout(()=>{this.copied=!1},1800)})}},onMount(){const e=this.$refs.el;e&&(this._cleanCode=ao(this.code),e.textContent=this._cleanCode,no.highlightElement(e))}},ve="Courvux",co="https://vanjexdev.github.io/courvux";function Z({title:e,description:t,slug:o="/"}){const i=e?`${e} — ${ve}`:`${ve} — Lightweight reactive UI framework`,s=co+o;return Zt({title:e??`${ve} — Lightweight reactive UI framework`,titleTemplate:e?`%s — ${ve}`:void 0,meta:[{name:"description",content:t},{property:"og:title",content:i},{property:"og:description",content:t},{property:"og:type",content:"website"},{property:"og:url",content:s},{property:"og:site_name",content:ve},{name:"twitter:card",content:"summary_large_image"},{name:"twitter:title",content:i},{name:"twitter:description",content:t}],link:[{rel:"canonical",href:s}]})}const lo={data:{install:`# From GitHub — pin a tag for stable installs
...
createApp({
    ...
    `,exprs:{"'bash'":(e=>"bash"),install:(e=>e.install),"'terminal'":(e=>"terminal"),"'js'":(e=>"js"),counter:(e=>e.counter),"'main.js'":(e=>"main.js")},onMount(){Z({description:"Lightweight reactive UI framework for the browser. No virtual DOM. Proxy-based reactivity. ~20 KB gzip with everything (router, store, devtools, composables, useHead, SSR).",slug:"/"})}},uo={data:{s1:`# Latest commit on main (rolling)
...
createApp({ store, router, template: '<router-view />' }).mount('#app');`},template:`
    ...
    `,exprs:{"'js'":(e=>"js"),s1:(e=>e.s1),"'main.js'":(e=>"main.js"),s2:(e=>e.s2),s3:(e=>e.s3),s4:(e=>e.s4)},onMount(){Z({title:"Quick Start",description:"Build your first reactive Courvux app — counter example, methods, computed properties.",slug:"/quick-start"})}},mo={data:{s_interp:`<!-- Text interpolation -->
<p>{{ count }}</p>
<p>{{ price * qty }}</p>
<p>{{ active ? 'On' : 'Off' }}</p>
...
<div id="app" cv-cloak></div>`},template:`
    ...
    `,exprs:{"'html'":(e=>"html"),s_interp:(e=>e.s_interp),s_bind:(e=>e.s_bind),s_events:(e=>e.s_events),s_cvfor:(e=>e.s_cvfor),s_cvif:(e=>e.s_cvif),s_cvshow:(e=>e.s_cvshow),s_cvmodel:(e=>e.s_cvmodel),s_cvhtml:(e=>e.s_cvhtml),s_cvdata:(e=>e.s_cvdata),s_misc:(e=>e.s_misc)},onMount(){Z({title:"Template Syntax",description:"Courvux directives and bindings: cv-if, cv-for, cv-model, cv-show, :class, :style, @event.",slug:"/template"})}},ho={data:{s_define:`createApp({
    components: {
        'user-card': {
...
}`,s3:`// Error boundary — catches errors from descendant onMount
{
    ...
    `,exprs:{"'js'":(e=>"js"),s1:(e=>e.s1),s2:(e=>e.s2),s3:(e=>e.s3)},onMount(){Z({title:"Lifecycle Hooks",description:"onMount, onBeforeUnmount, onDestroy, error boundaries, and async data in Courvux.",slug:"/lifecycle"})}},vo={data:{s_storage:`import { cvStorage } from 'courvux';
...
bus.emit('toast:show', 'Saved again');  // handler does NOT run`},template:`
    ...
    `,exprs:{"'js'":(e=>"js"),s_basic:(e=>e.s_basic),"'ts'":(e=>"ts"),s_typed:(e=>e.s_typed),s_once:(e=>e.s_once),s_provide:(e=>e.s_provide),s_inject:(e=>e.s_inject)},onMount(){Z({title:"Event Bus",description:"Typed cross-component event bus in Courvux: on, off, emit, once, clear, and provide/inject patterns.",slug:"/event-bus"})}},yo={data:{s_setup:`import { createApp, createRouter } from 'courvux';
...
methods: {
    ...
    `,exprs:{"'js'":(e=>"js"),s_setup:(e=>e.s_setup),s_route:(e=>e.s_route),s_params:(e=>e.s_params),s_lazy:(e=>e.s_lazy),s_navigate:(e=>e.s_navigate),s_nested:(e=>e.s_nested)},onMount(){Z({title:"Router",description:"SPA routing in Courvux: dynamic params, nested routes, navigation guards, transitions.",slug:"/router"})}},xo={data:{s1:`import { createStore } from 'courvux';
...
{ inject: { localTheme: 'theme', endpoint: 'apiUrl' } }`},template:`
    ...
    `,exprs:{"'js'":(e=>"js"),s1:(e=>e.s1),"'html'":(e=>"html"),s2:(e=>e.s2),s3:(e=>e.s3),s4:(e=>e.s4),s5:(e=>e.s5),s6:(e=>e.s6)},onMount(){Z({title:"Store",description:"Global reactive state in Courvux with createStore, modules, and namespaced actions.",slug:"/store"})}},wo={data:{s_basic:`import { useHead } from 'courvux';
...
// → embed headHtml in your shell, then html in the mount root`},template:`
    ...
    mode: 'history',
    base: '/courvux',
});`)},onMount(){Z({title:"Static Site Generation",description:"Pre-render every route to static HTML at build time with the courvux/plugin/ssg Vite plugin.",slug:"/ssg"})}},So={data:{s_setup:`import { createApp, setupDevTools, mountDevOverlay } from 'courvux';
...
expect(w.text()).toBe('Alice');`,s_async:`it('waits for async data', async () => {
    ...
    `,exprs:{"'js'":(e=>"js"),s_config:(e=>e.s_config),s_basic:(e=>e.s_basic),s_state:(e=>e.s_state),s_global:(e=>e.s_global),s_async:(e=>e.s_async)},onMount(){Z({title:"Testing",description:"Vitest-compatible test utility for Courvux. Mount components, drive state, query the DOM with happy-dom.",slug:"/testing"})}},_o={data:{s_manifest:`{
  "name":             "My App",
  "short_name":       "MyApp",
...
this.cards.splice(idx, 1);`},template:`
    ...
    `,exprs:{"'html'":(e=>"html"),s_keyed:(e=>e.s_keyed),"item.id":(e=>e.item.id),"'js'":(e=>"js"),s_csp:(e=>e.s_csp),"'ts'":(e=>"ts"),s_ts:(e=>e.s_ts),s_lazy:(e=>e.s_lazy),s_vite:(e=>e.s_vite),s_tauri:(e=>e.s_tauri),s_test:(e=>e.s_test),s_proxyId_faq:(e=>e.s_proxyId_faq)},onMount(){Z({title:"FAQ",description:"Frequently asked questions and troubleshooting for Courvux: cv-for keys, strict CSP, TypeScript, lazy loading, Vite/Tauri integration, testing, migration.",slug:"/faq"})}},$o={data:{s_component_vue:`// Vue 3 — Options API
export default {
    data() { return { count: 0 } },
...
<button cv-show="todos.some(t => t.done)" @click="clearCompleted()">Clear completed</button>`;function Po(){try{return JSON.parse(localStorage.getItem(kt))||[]}catch{return[]}}function Do(e){localStorage.setItem(kt,JSON.stringify(e))}const Io={data:{todos:Po(),newTodo:"",filter:"all",editingId:null,editText:"",_nextId:Date.now(),srcJs:Oo,srcHtml:Fo,srcTab:"js"},computed:{filteredTodos(){return this.filter==="active"?this.todos.filter(e=>!e.done):this.filter==="completed"?this.todos.filter(e=>e.done):this.todos},remaining(){return this.todos.filter(e=>!e.done).length},allDone(){return this.todos.length>0&&this.todos.every(e=>e.done)}},watch:{todos:{deep:!0,handler(e){Do(e)}}},methods:{add(){const e=this.newTodo.trim();e&&(this.todos=[...this.todos,{id:this._nextId++,text:e,done:!1}],this.newTodo="")},toggle(e){this.todos=this.todos.map(t=>t.id===e?{...t,done:!t.done}:t)},remove(e){this.todos=this.todos.filter(t=>t.id!==e)},clearCompleted(){this.todos=this.todos.filter(e=>!e.done)},toggleAll(){const e=this.allDone;this.todos=this.todos.map(t=>({...t,done:!e}))},startEdit(e){this.editingId=e.id,this.editText=e.text,this.$nextTick(()=>{var t;return(t=this.$refs["edit_"+e.id])==null?void 0:t.focus()})},commitEdit(e){const t=this.editText.trim();t?this.todos=this.todos.map(o=>o.id===e?{...o,text:t}:o):this.remove(e),this.editingId=null,this.editText=""},cancelEdit(){this.editingId=null,this.editText=""},setFilter(e){this.filter=e}},template:`
