...
`,Ue=!1;function Be(){if(Ue||typeof document>"u")return;Ue=!0;const e=document.createElement("style");e.id="cv-transitions-el",e.textContent=xt,document.head.appendChild(e)}var qe=!1;function wt(){if(qe||typeof document>"u")return;qe=!0;const e=document.createElement("style");e.id="cv-cloak-style",e.textContent="[cv-cloak]{display:none!important}",document.head.appendChild(e)}function kt(e){if(typeof window<"u"&&"Sanitizer"in window){const o=document.createElement("div");return o.setHTML(e,{sanitizer:new window.Sanitizer}),o.innerHTML}const t=new DOMParser().parseFromString(e,"text/html");return t.querySelectorAll("script,iframe,object,embed,form,meta,link,style").forEach(o=>o.remove()),t.querySelectorAll("*").forEach(o=>{Array.from(o.attributes).forEach(c=>{(c.name.startsWith("on")||c.value.trim().toLowerCase().startsWith("javascript:"))&&o.removeAttribute(c.name)})}),t.body.innerHTML}async function oe(e,t,o){var y,E,x,D,T,k,V,j,H,R,C,P,_,d,a,v,S;const c=Array.from(e.childNodes);let l=0;for(;l<c.length;){const A=c[l];if(A.nodeType===3){const i=A.textContent||"",r=i.match(/\{\{([\s\S]+?)\}\}/g);if(r){const s=i,u=()=>{let m=s;r.forEach(p=>{const b=p.replace(/^\{\{\s*/,"").replace(/\s*\}\}$/,"");m=m.replace(p,N(b,t)??"")}),A.textContent=m};r.forEach(m=>{K(m.replace(/^\{\{\s*/,"").replace(/\s*\}\}$/,""),o,u)}),u()}l++;continue}if(A.nodeType!==1){l++;continue}const n=A,h=n.tagName.toLowerCase();if(n.hasAttribute("cv-pre")){n.removeAttribute("cv-pre"),l++;continue}if(n.hasAttribute("cv-once")){n.removeAttribute("cv-once"),await oe(n,t,{...o,subscribe:()=>()=>{},storeSubscribeOverride:()=>()=>{}}),l++;continue}if(n.hasAttribute("cv-cloak")&&n.removeAttribute("cv-cloak"),n.hasAttribute("cv-teleport")){const i=n.getAttribute("cv-teleport");n.removeAttribute("cv-teleport");const r=document.querySelector(i)??document.body,s=document.createComment(`cv-teleport: ${i}`);n.replaceWith(s),await oe(n,t,o),r.appendChild(n),l++;continue}if(n.hasAttribute("cv-memo")){const i=n.getAttribute("cv-memo");n.removeAttribute("cv-memo");const r=()=>i.split(",").map(b=>N(b.trim(),t));let s=r();const u=[],m=b=>(u.push(b),()=>{const g=u.indexOf(b);g>-1&&u.splice(g,1)});await oe(n,t,{...o,subscribe:(b,g)=>m(g),storeSubscribeOverride:(b,g,w)=>m(w)});const p=K(i,o,()=>{const b=r();b.some((g,w)=>g!==s[w])&&(s=b,[...u].forEach(g=>g()))});(y=o.registerCleanup)==null||y.call(o,()=>p()),l++;continue}if(n.hasAttribute("cv-data")){const i=n.getAttribute("cv-data").trim();n.removeAttribute("cv-data");let r={},s={};if(i.startsWith("{")){const u=N(i,t)??{};Object.entries(u).forEach(([m,p])=>{typeof p=="function"?s[m]=p:r[m]=p})}else if(i){const u=(E=o.components)==null?void 0:E[i];if(u){const m=typeof u.data=="function"?u.data():u.data??{};m instanceof Promise||Object.assign(r,m),Object.assign(s,u.methods??{})}}if(o.createChildScope){const u=o.createChildScope(r,s);(x=o.registerCleanup)==null||x.call(o,u.cleanup),await oe(n,u.state,{...o,subscribe:u.subscribe})}else await oe(n,{...t,...r,...s},o);l++;continue}if(n.hasAttribute("cv-for")){const i=n.getAttribute("cv-for");n.removeAttribute("cv-for");const r=i.match(/^\(?(\w+)(?:,\s*(\w+))?\)?\s+in\s+(.+)$/);if(r){const[,s,u,m]=r,p=n.getAttribute(":key")??null;p&&n.removeAttribute(":key");const b=n.getAttribute("cv-transition")??null;b&&n.removeAttribute("cv-transition");const g=document.createComment(`cv-for: ${m}`);n.replaceWith(g);let w=[],L=[];const F=new Map,W=async()=>{var G;const z=N(m,t),M=z?typeof z=="number"?Array.from({length:z},(I,B)=>[B+1,B]):Array.isArray(z)?z.map((I,B)=>[I,B]):Object.entries(z).map(([I,B])=>[B,I]):[];if(p){const I=[],B=new Map,Q=new Set;for(const[Y,X]of M){const re=N(p,He(t,Y,s,X,u));Q.has(re)&&console.warn(`[courvux] cv-for: duplicate :key "${re}" in "${m}"`),Q.add(re),I.push(re),B.set(re,[Y,X])}const q=[];for(const[Y,{el:X,destroy:re}]of F)B.has(Y)||(b?(X.classList.add(`${b}-leave`),q.push(ie(X).then(()=>{var ae;X.classList.remove(`${b}-leave`),re(),(ae=X.parentNode)==null||ae.removeChild(X),F.delete(Y)}))):(re(),(G=X.parentNode)==null||G.removeChild(X),F.delete(Y)));q.length&&await Promise.all(q);const U=g.parentNode,te=[];for(const Y of I){const[X,re]=B.get(Y);if(F.has(Y)){const ae=F.get(Y);ae.itemRef!==X&&(ae.reactive[s]=X,ae.itemRef=X),u&&(ae.reactive[u]=re)}else{const ae=n.cloneNode(!0),Se=[],{subscribe:ht,createReactiveState:ft}=Pe(),Ee=ft({[s]:X,...u?{[u]:re}:{}}),vt=new Proxy({},{has(se,ee){return!0},get(se,ee){return typeof ee!="string"?t[ee]:ee===s||u&&ee===u?Ee[ee]:t[ee]},set(se,ee,ce){return ee===s||u&&ee===u?(Ee[ee]=ce,!0):(t[ee]=ce,!0)}}),gt={...o,subscribe:(se,ee)=>{const ce=se.split(".")[0];let de;return ce===s||u&&ce===u?de=ht(ce,ee):de=o.subscribe(se,ee),Se.push(de),de},storeSubscribeOverride:(se,ee,ce)=>{const de=ve(se,ee,ce);return Se.push(de),de}},_e=document.createDocumentFragment();_e.appendChild(ae),await oe(_e,vt,gt);const $e=_e.firstChild??ae;b&&$e.classList.add(`${b}-enter`),F.set(Y,{el:$e,reactive:Ee,itemRef:X,destroy:()=>Se.forEach(se=>se())}),b&&te.push($e)}}let J=g.nextSibling,pe=0;for(const Y of I){const{el:X}=F.get(Y);X!==J?pe++:J=X.nextSibling}if(pe>0)if(pe>I.length>>1){const Y=document.createDocumentFragment();for(const X of I)Y.appendChild(F.get(X).el);U.insertBefore(Y,g.nextSibling)}else{J=g.nextSibling;for(const Y of I){const{el:X}=F.get(Y);X!==J?U.insertBefore(X,J):J=X.nextSibling}}w=I.map(Y=>F.get(Y).el),te.length&&Promise.all(te.map(Y=>ie(Y).then(()=>Y.classList.remove(`${b}-enter`))))}else{if(L.forEach(q=>q()),L=[],w.forEach(q=>{var U;return(U=q.parentNode)==null?void 0:U.removeChild(q)}),w=[],!M.length)return;const I=g.parentNode,B=g.nextSibling,Q={...o,subscribe:(q,U)=>{const te=o.subscribe(q,U);return L.push(te),te},storeSubscribeOverride:(q,U,te)=>{const J=ve(q,U,te);return L.push(J),J}};for(const[q,U]of M){const te=n.cloneNode(!0),J=document.createDocumentFragment();J.appendChild(te),await oe(J,He(t,q,s,U,u),Q);const pe=J.firstChild??te;I.insertBefore(J,B),w.push(pe)}}};(D=o.registerCleanup)==null||D.call(o,()=>{F.forEach(({el:z,destroy:M})=>{var G;M(),(G=z.parentNode)==null||G.removeChild(z)}),F.clear(),L.forEach(z=>z()),w.forEach(z=>{var M;return(M=z.parentNode)==null?void 0:M.removeChild(z)}),w=[]}),K(m,o,W),await W()}l++;continue}if(n.hasAttribute("cv-if")){const i=[],r=n.getAttribute("cv-if");n.removeAttribute("cv-if");const s=document.createComment("cv-if");n.replaceWith(s),i.push({condition:r,template:n,anchor:s});let u=l+1;for(;u<c.length;){const w=c[u];if(w.nodeType===3&&(((T=w.textContent)==null?void 0:T.trim())??"")===""){u++;continue}if(w.nodeType!==1)break;const L=w;if(L.hasAttribute("cv-else-if")){const F=L.getAttribute("cv-else-if");L.removeAttribute("cv-else-if");const W=document.createComment("cv-else-if");L.replaceWith(W),i.push({condition:F,template:L,anchor:W}),u++;continue}if(L.hasAttribute("cv-else")){L.removeAttribute("cv-else");const F=document.createComment("cv-else");L.replaceWith(F),i.push({condition:null,template:L,anchor:F}),u++;break}break}l=u;let m=null,p=!1,b=!1;const g=async()=>{var w,L;if(p){b=!0;return}p=!0;try{do{b=!1,m&&((w=m.parentNode)==null||w.removeChild(m),m=null);for(const F of i)if(F.condition===null||N(F.condition,t)){const W=F.template.cloneNode(!0),z=document.createDocumentFragment();z.appendChild(W),await oe(z,t,o);const M=z.firstChild??W;(L=F.anchor.parentNode)==null||L.insertBefore(z,F.anchor.nextSibling),m=M;break}}while(b)}finally{p=!1}};i.filter(w=>w.condition).forEach(w=>{K(w.condition,o,g)}),await g();continue}if(n.hasAttribute("cv-show")){const i=n.getAttribute("cv-show");n.removeAttribute("cv-show");const r=Array.from(n.attributes).filter(s=>s.name==="cv-transition"||s.name.startsWith("cv-transition:")||s.name.startsWith("cv-transition."));if(r.length>0){const s=M=>(n.getAttribute(M)??"").split(" ").filter(Boolean),u=s("cv-transition:enter"),m=s("cv-transition:enter-start"),p=s("cv-transition:enter-end"),b=s("cv-transition:leave"),g=s("cv-transition:leave-start"),w=s("cv-transition:leave-end"),L=n.getAttribute("cv-transition")??"",F=new Set(L.split(".").slice(1)),W=[...F].find(M=>/^\d+$/.test(M)),z=W?parseInt(W):200;if(u.length||m.length||b.length||g.length){r.forEach(q=>n.removeAttribute(q.name));const M=()=>new Promise(q=>requestAnimationFrame(()=>requestAnimationFrame(()=>q())));let G=!!N(i,t),I=!1,B=null;const Q=async q=>{if(I){B=q;return}I=!0;try{q?(n.style.display="",n.classList.add(...u,...m),await M(),n.classList.remove(...m),n.classList.add(...p),await ie(n),n.classList.remove(...u,...p)):(n.classList.add(...b,...g),await M(),n.classList.remove(...g),n.classList.add(...w),await ie(n),n.classList.remove(...b,...w),n.style.display="none"),G=q}finally{if(I=!1,B!==null&&B!==G){const U=B;B=null,Q(U)}else B=null}};G||(n.style.display="none"),K(i,o,()=>{const q=!!N(i,t);q!==G&&Q(q)})}else{const M=[...F].find(U=>U==="scale"||/^scale$/.test(U)),G=(()=>{const U=[...F].find(te=>/^\d+$/.test(te)&&te!==W);return U?parseInt(U)/100:.9})(),I=[];(!F.has("scale")||F.has("opacity"))&&I.push(`opacity ${z}ms ease`),M&&I.push(`transform ${z}ms ease`),I.length||I.push(`opacity ${z}ms ease`),n.style.transition=(n.style.transition?n.style.transition+", ":"")+I.join(", "),r.forEach(U=>n.removeAttribute(U.name));let B=!!N(i,t);const Q=()=>new Promise(U=>requestAnimationFrame(()=>requestAnimationFrame(()=>U()))),q=async U=>{U?(n.style.display="",n.style.opacity="0",M&&(n.style.transform=`scale(${G})`),await Q(),n.style.opacity="",M&&(n.style.transform=""),await ie(n)):(n.style.opacity="0",M&&(n.style.transform=`scale(${G})`),await ie(n),n.style.display="none",n.style.opacity="",M&&(n.style.transform="")),B=U};B||(n.style.display="none"),K(i,o,()=>{const U=!!N(i,t);U!==B&&q(U)})}}else{const s=n.getAttribute("cv-show-transition"),u=n.getAttribute(":transition");s&&n.removeAttribute("cv-show-transition"),u&&n.removeAttribute(":transition");const m=s??(u?String(N(u,t)):null);if(m){Be();let p=!!N(i,t);p||(n.style.display="none");let b=!1,g=null;const w=async L=>{if(b){g=L;return}b=!0;try{L?(n.style.display="",n.classList.add(`${m}-enter`),await ie(n),n.classList.remove(`${m}-enter`)):(n.classList.add(`${m}-leave`),await ie(n),n.classList.remove(`${m}-leave`),n.style.display="none"),p=L}finally{if(b=!1,g!==null&&g!==p){const F=g;g=null,w(F)}else g=null}};K(i,o,()=>{const L=!!N(i,t);L!==p&&w(L)})}else{const p=()=>{n.style.display=N(i,t)?"":"none"};K(i,o,p),p()}}}if(n.hasAttribute("cv-focus")){const i=n.getAttribute("cv-focus")??"";if(n.removeAttribute("cv-focus"),!i)Promise.resolve().then(()=>n.focus());else{const r=()=>{N(i,t)&&Promise.resolve().then(()=>n.focus())};K(i,o,r),r()}}{const i=Array.from(n.attributes).filter(r=>r.name==="cv-intersect"||r.name.startsWith("cv-intersect:")||r.name.startsWith("cv-intersect."));if(i.length&&typeof IntersectionObserver<"u"){const r=i.find(M=>M.name==="cv-intersect"||M.name==="cv-intersect:enter"||M.name.startsWith("cv-intersect.")),s=i.find(M=>M.name==="cv-intersect:leave"),u=(r==null?void 0:r.value)??"",m=(s==null?void 0:s.value)??"",p=((r==null?void 0:r.name)??"cv-intersect").split("."),b=new Set(p.slice(1)),g=b.has("once");let w=0;if(b.has("half"))w=.5;else if(b.has("full"))w=1;else{const M=[...b].find(G=>G.startsWith("threshold-"));M&&(w=parseInt(M.replace("threshold-",""))/100)}const L=[...b].find(M=>M.startsWith("margin-")),F=L?`${L.replace("margin-","")}px`:void 0;i.forEach(M=>n.removeAttribute(M.name));const W=M=>{if(M)try{new Function("$data",`with($data){${M}}`)(t)}catch(G){console.warn(`[courvux] cv-intersect error "${M}":`,G)}},z=new IntersectionObserver(M=>{M.forEach(G=>{G.isIntersecting?(W(u),g&&z.disconnect()):W(m)})},{threshold:w,...F?{rootMargin:F}:{}});z.observe(n),(k=o.registerCleanup)==null||k.call(o,()=>z.disconnect())}}{const i=Array.from(n.attributes).find(r=>r.name==="cv-html"||r.name.startsWith("cv-html."));if(i){const r=i.value;n.removeAttribute(i.name);const s=i.name.split(".").slice(1).includes("sanitize"),u=()=>{const m=String(N(r,t)??"");n.innerHTML=s?kt(m):m};K(r,o,u),u(),l++;continue}}if(n.hasAttribute("cv-ref")&&!((V=o.components)!=null&&V[h])){const i=n.getAttribute("cv-ref");n.removeAttribute("cv-ref"),o.refs&&(o.refs[i]=n)}const f=!!((j=o.components)!=null&&j[h]),$=Array.from(n.attributes).find(i=>i.name==="cv-model"||i.name.startsWith("cv-model."));if($&&!f){const i=$.value;n.removeAttribute($.name);const r=new Set($.name.split(".").slice(1)),s=n,u=(H=s.type)==null?void 0:H.toLowerCase(),m=p=>{if(r.has("number")){const b=parseFloat(p);return isNaN(b)?p:b}return r.has("trim")?p.trim():p};if(u==="checkbox"){const p=()=>{const b=N(i,t);s.checked=Array.isArray(b)?b.includes(s.value):!!b};ue(i,o,p),p(),s.addEventListener("change",()=>{const b=N(i,t);if(Array.isArray(b)){const g=[...b];if(s.checked)g.includes(s.value)||g.push(s.value);else{const w=g.indexOf(s.value);w>-1&&g.splice(w,1)}le(i,t,g)}else le(i,t,s.checked)})}else if(u==="radio"){const p=()=>{s.checked=N(i,t)===s.value};ue(i,o,p),p(),s.addEventListener("change",()=>{s.checked&&le(i,t,m(s.value))})}else if(n.hasAttribute("contenteditable")){const p=n,b=()=>{const g=String(N(i,t)??"");p.innerText!==g&&(p.innerText=g)};if(ue(i,o,b),b(),r.has("debounce")){const g=[...r].find(F=>/^\d+$/.test(F)),w=g?parseInt(g):300;let L;p.addEventListener("input",()=>{clearTimeout(L),L=setTimeout(()=>le(i,t,m(p.innerText)),w)})}else{const g=r.has("lazy")?"blur":"input";p.addEventListener(g,()=>le(i,t,m(p.innerText)))}}else{const p=()=>{s.value=N(i,t)??""};if(ue(i,o,p),p(),r.has("debounce")){const b=[...r].find(L=>/^\d+$/.test(L)),g=b?parseInt(b):300;let w;s.addEventListener("input",()=>{clearTimeout(w),w=setTimeout(()=>le(i,t,m(s.value)),g)})}else{const b=h==="select"||r.has("lazy")?"change":"input";s.addEventListener(b,()=>le(i,t,m(s.value)))}}}if(o.directives&&Array.from(n.attributes).forEach(i=>{var W,z;if(!i.name.startsWith("cv-"))return;const r=i.name.slice(3).split("."),s=r[0],u=r.slice(1),m=s.indexOf(":"),p=m>=0?s.slice(0,m):s,b=m>=0?s.slice(m+1):void 0,g=o.directives[p];if(!g)return;const w=i.value;n.removeAttribute(i.name);const L=typeof g=="function"?{onMount:g}:g,F={value:w?N(w,t):void 0,arg:b,modifiers:Object.fromEntries(u.map(M=>[M,!0]))};(W=L.onMount)==null||W.call(L,n,F),L.onUpdate&&w&&K(w,o,()=>{F.value=N(w,t),L.onUpdate(n,F)}),L.onDestroy&&((z=o.registerCleanup)==null||z.call(o,()=>L.onDestroy(n,F)))}),h==="slot"){const i=n.getAttribute("name")??"default",r=(R=o.slots)==null?void 0:R[i];if(r){const s={};Array.from(n.attributes).forEach(p=>{p.name.startsWith(":")&&(s[p.name.slice(1)]=N(p.value,t))});const u=await r(s),m=document.createDocumentFragment();u.forEach(p=>m.appendChild(p)),n.replaceWith(m)}else{const s=document.createDocumentFragment();for(;n.firstChild;)s.appendChild(n.firstChild);await oe(s,t,o),n.replaceWith(s)}l++;continue}if(h==="cv-transition"){Be();const i=n.getAttribute("name")??"fade",r=n.getAttribute(":show")??null;n.removeAttribute("name"),r&&n.removeAttribute(":show");const s=document.createElement("div");for(s.className="cv-t-wrap";n.firstChild;)s.appendChild(n.firstChild);if(n.replaceWith(s),await oe(s,t,o),r){let u=!!N(r,t),m=!1,p=null;u||(s.style.display="none");const b=async g=>{if(m){p=g;return}m=!0;try{g?(s.style.display="",s.classList.add(`${i}-enter`),await ie(s),s.classList.remove(`${i}-enter`)):(s.classList.add(`${i}-leave`),await ie(s),s.classList.remove(`${i}-leave`),s.style.display="none"),u=g}finally{if(m=!1,p!==null&&p!==u){const w=p;p=null,b(w)}else p=null}};K(r,o,()=>{const g=!!N(r,t);g!==u&&b(g)})}l++;continue}if(h==="router-view"&&o.mountRouterView){const i=n.getAttribute("name")??void 0;n.setAttribute("aria-live","polite"),n.setAttribute("aria-atomic","true"),await o.mountRouterView(n,i),l++;continue}if(h==="router-link"){const i=n.getAttribute(":to"),r=n.getAttribute("to"),s=()=>i?String(N(i,t)??"/"):r||"/",u=document.createElement("a");u.innerHTML=n.innerHTML,Array.from(n.attributes).forEach(w=>{w.name!=="to"&&w.name!==":to"&&u.setAttribute(w.name,w.value)});const m=((C=o.router)==null?void 0:C.base)??"",p=w=>m?w===m?"/":w.startsWith(m+"/")?w.slice(m.length)||"/":w||"/":w||"/",b=()=>{var w;return((w=o.router)==null?void 0:w.mode)==="history"?p(window.location.pathname):window.location.hash.slice(1)||"/"},g=()=>{var F;const w=s(),L=b()===w;((F=o.router)==null?void 0:F.mode)==="history"?u.href=`${m}${w}`:u.href=`#${w}`,L?(u.setAttribute("aria-current","page"),u.classList.add("active")):(u.removeAttribute("aria-current"),u.classList.remove("active"))};((P=o.router)==null?void 0:P.mode)==="history"?(u.addEventListener("click",w=>{w.preventDefault(),o.router.navigate(s())}),window.addEventListener("popstate",g)):window.addEventListener("hashchange",g),i&&ue(i,o,g),g(),n.replaceWith(u),await oe(u,t,o),l++;continue}if(h==="component"&&n.hasAttribute(":is")&&o.mountDynamic){const i=n.getAttribute(":is");n.removeAttribute(":is");const r=document.createComment("component:is");n.replaceWith(r),await o.mountDynamic(r,i,n,t,o),l++;continue}if((_=o.components)!=null&&_[h]&&o.mountElement){await o.mountElement(n,h,t,o),l++;continue}{const i=Array.from(n.attributes).find(r=>r.name==="cv-intersect"||r.name.startsWith("cv-intersect."));if(i&&typeof IntersectionObserver<"u"){const r=new Set(i.name.split(".").slice(1));n.removeAttribute(i.name);const s=N(i.value,t);let u,m=0,p="0px",b=r.has("once");if(typeof s=="function"?u=g=>s.call(t,g):s&&typeof s=="object"&&(typeof s.handler=="function"&&(u=g=>s.handler.call(t,g)),s.threshold!==void 0&&(m=s.threshold),s.margin&&(p=s.margin),s.once&&(b=!0)),u){const g=new IntersectionObserver(w=>{const L=w[0];u(L),b&&L.isIntersecting&&g.disconnect()},{threshold:m,rootMargin:p});g.observe(n),(d=o.registerCleanup)==null||d.call(o,()=>g.disconnect())}}}if(n.hasAttribute("cv-resize")){const i=n.getAttribute("cv-resize");if(n.removeAttribute("cv-resize"),typeof ResizeObserver<"u"){const r=N(i,t);let s,u="content-box";if(typeof r=="function"?s=m=>r.call(t,m):r&&typeof r=="object"&&(typeof r.handler=="function"&&(s=m=>r.handler.call(t,m)),r.box&&(u=r.box)),s){const m=new ResizeObserver(p=>{p[0]&&s(p[0])});m.observe(n,{box:u}),(a=o.registerCleanup)==null||a.call(o,()=>m.disconnect())}}}if(n.hasAttribute("cv-scroll")){const i=n.getAttribute("cv-scroll");n.removeAttribute("cv-scroll");const r=N(i,t);let s,u=0;if(typeof r=="function"?s=m=>r.call(t,m):r&&typeof r=="object"&&(typeof r.handler=="function"&&(s=m=>r.handler.call(t,m)),r.throttle&&(u=r.throttle)),s){let m=0;const p=()=>{const b=Date.now();u>0&&b-m<u||(m=b,s({scrollTop:n.scrollTop,scrollLeft:n.scrollLeft,scrollHeight:n.scrollHeight,scrollWidth:n.scrollWidth,clientHeight:n.clientHeight,clientWidth:n.clientWidth}))};n.addEventListener("scroll",p,{passive:!0}),(v=o.registerCleanup)==null||v.call(o,()=>n.removeEventListener("scroll",p))}}if(n.hasAttribute("cv-clickoutside")){const i=n.getAttribute("cv-clickoutside");n.removeAttribute("cv-clickoutside");const r=s=>{n.contains(s.target)||(typeof t[i]=="function"?t[i].call(t,s):We(i,t,s))};document.addEventListener("click",r,!0),(S=o.registerCleanup)==null||S.call(o,()=>document.removeEventListener("click",r,!0))}if(n.hasAttribute("cv-bind")){const i=n.getAttribute("cv-bind");n.removeAttribute("cv-bind");const r=n.getAttribute("class")??"",s=n.getAttribute("style")??"";let u=[];const m=()=>{const p=N(i,t)??{},b=Object.keys(p);for(const g of u)g in p||(g==="class"?n.className=r:g==="style"?n.style.cssText=s:n.removeAttribute(g));for(const[g,w]of Object.entries(p))g==="class"?n.className=[r,De(w)].filter(Boolean).join(" "):g==="style"?Ne(n,w,s):w==null||w===!1?n.removeAttribute(g):n.setAttribute(g,w===!0?"":String(w));u=b};K(i,o,m),m()}const O={enter:"Enter",esc:"Escape",escape:"Escape",space:" ",tab:"Tab",delete:"Delete",backspace:"Backspace",up:"ArrowUp",down:"ArrowDown",left:"ArrowLeft",right:"ArrowRight"};Array.from(n.attributes).forEach(i=>{if(i.name.startsWith("@")||i.name.startsWith("cv:on:")){const r=(i.name.startsWith("@")?i.name.substring(1):i.name.substring(6)).split("."),s=r[0],u=new Set(r.slice(1)),m=[...u].find(w=>w in O),p=i.value,b=w=>{u.has("prevent")&&w.preventDefault(),u.has("stop")&&w.stopPropagation(),!(u.has("self")&&w.target!==w.currentTarget)&&(m&&w.key!==O[m]||(typeof t[p]=="function"?t[p].call(t,w):We(p,t,w)))},g={};u.has("once")&&(g.once=!0),u.has("passive")&&(g.passive=!0),u.has("capture")&&(g.capture=!0),n.addEventListener(s,b,Object.keys(g).length?g:void 0)}else if(i.name.startsWith(":")){const r=i.name.slice(1),s=i.value;if(r==="class"){const u=n.getAttribute("class")??"",m=()=>{n.className=[u,De(N(s,t))].filter(Boolean).join(" ")};K(s,o,m),m()}else if(r==="style"){const u=n.getAttribute("style")??"",m=()=>Ne(n,N(s,t),u);K(s,o,m),m()}else if(r.includes("-")){const u=()=>{const m=N(s,t);m==null||m===!1?n.removeAttribute(r):n.setAttribute(r,m===!0?"":String(m))};K(s,o,u),u()}else{const u=()=>{n[r]=N(s,t)??""};K(s,o,u),u()}}}),A.hasChildNodes()&&await oe(A,t,o),l++}}var At=`
...
`;function St(){if(document.getElementById("cv-transitions"))return;const e=document.createElement("style");e.id="cv-transitions",e.textContent=At,document.head.appendChild(e)}async function ge(e,t,o){e.classList.add(`${t}-${o}`);const c=getComputedStyle(e),l=Math.max(parseFloat(c.animationDuration)||0,parseFloat(c.transitionDuration)||0)*1e3;l>0&&await new Promise(y=>{const E=()=>y();e.addEventListener("animationend",E,{once:!0}),e.addEventListener("transitionend",E,{once:!0}),setTimeout(E,l+50)}),e.classList.remove(`${t}-${o}`)}var we=new Map;async function Et(e){if(typeof e!="function")return e;if(we.has(e))return we.get(e);const t=await e();return we.set(e,t.default),t.default}function Ve(e,t){if(e.components)return e.components[t];if(t==="default")return e.component}function Ge(e,t){if(e==="*")return{};const o=[],c=e.replace(/:(\w+)/g,(y,E)=>(o.push(E),"([^/]+)")),l=t.match(new RegExp(`^${c}$`));return l?Object.fromEntries(o.map((y,E)=>[y,l[E+1]])):null}function _t(e,t){if(e==="/")return{params:{},remaining:t};const o=[],c=e.replace(/:(\w+)/g,(y,E)=>(o.push(E),"([^/]+)")),l=t.match(new RegExp(`^${c}(/.+)?$`));return l?{params:Object.fromEntries(o.map((y,E)=>[y,l[E+1]])),remaining:l[o.length+1]||"/"}:null}function ut(e,t=""){return e.map(o=>{var l;if(o.path==="*")return o;const c=((t.endsWith("/")?t.slice(0,-1):t)+o.path).replace(/\/+/g,"/")||"/";return(l=o.children)!=null&&l.length?{...o,path:c,children:ut(o.children,c==="/"?"":c)}:{...o,path:c}})}var me=(e,t)=>new Promise(o=>e(t,o)),Xe=(e,t)=>e!=null&&e.beforeLeave?new Promise(o=>e.beforeLeave(t,o)):Promise.resolve(void 0);function $t(e,t={}){const o=t.mode??"hash",c=Tt(t.base??"");return{routes:ut(e),mode:o,base:c,transition:t.transition,beforeEach:t.beforeEach,afterEach:t.afterEach,scrollBehavior:t.scrollBehavior,navigate(l,y){const E=Ye(l,y==null?void 0:y.query);o==="history"?(history.pushState({},"",`${c}${E}`),window.dispatchEvent(new PopStateEvent("popstate"))):window.location.hash=E},replace(l,y){const E=Ye(l,y==null?void 0:y.query);if(o==="history")history.replaceState({},"",`${c}${E}`),window.dispatchEvent(new PopStateEvent("popstate"));else{const x=window.location.href.split("#")[0];window.location.replace(`${x}#${E}`)}},back(){history.back()},forward(){history.forward()}}}function Tt(e){if(!e||e==="/")return"";let t=e.startsWith("/")?e:`/${e}`;return t.endsWith("/")&&(t=t.slice(0,-1)),t}function Ct(e,t){return t?e===t?"/":e.startsWith(t+"/")?e.slice(t.length)||"/":e||"/":e||"/"}function Ye(e,t){return!t||!Object.keys(t).length?e:`${e}?${new URLSearchParams(t).toString()}`}function Ze(e){if(!e)return{};const t=new URLSearchParams(e.startsWith("?")?e.slice(1):e),o={};return t.forEach((c,l)=>{o[l]=c}),o}function Je(e,t,o,c="default",l){const y=t.base??"",E=()=>t.mode==="history"?Ct(window.location.pathname,y):(window.location.hash.slice(1)||"/").split("?")[0]||"/",x=()=>{if(t.mode==="history")return Ze(window.location.search);const a=window.location.hash.slice(1)||"/",v=a.indexOf("?");return v>=0?Ze(a.slice(v+1)):{}};t.transition&&St();let D=null,T=null,k=null,V=null,j=!1;const H=()=>{j||(j=!0,l==null||l())},R=new Map,C=a=>{var v;if(a!=null&&a.keepAlive&&k){(v=k.deactivate)==null||v.call(k);const S=document.createDocumentFragment();for(;e.firstChild;)S.appendChild(e.firstChild);R.set(D.path,{fragment:S,activation:k}),k=null}else k==null||k.destroy(),k=null,e.innerHTML=""},P=async(a,v,S,A,n)=>{const h=typeof v=="function"&&!we.has(v),f=h?v.__asyncOptions:void 0,$=a.loadingTemplate??(f==null?void 0:f.loadingTemplate);h&&$&&(e.innerHTML=$);let O;try{O=await Et(v)}catch(i){const r=f==null?void 0:f.errorTemplate;if(r)return e.innerHTML=r,{destroy:()=>{e.innerHTML=""}};throw i}return h&&$&&(e.innerHTML=""),o(e,O,S,A,n)},_=async()=>{var S,A,n,h,f,$,O,i;const a=E(),v=x();for(const r of t.routes){if((S=r.children)!=null&&S.length){const u=_t(r.path,a);if(u!==null)for(const m of r.children){const p=Ge(m.path,a);if(p!==null){const b={params:u.params,query:v,path:a,meta:r.meta};if(m.redirect){const F={params:p,query:v,path:a,meta:m.meta},W=typeof m.redirect=="function"?m.redirect(F):m.redirect;t.navigate(W);return}if(t.beforeEach){const F=await me(t.beforeEach,b);if(F){t.navigate(F);return}}if(r.beforeEnter){const F=await me(r.beforeEnter,b);if(F){t.navigate(F);return}}if(m.beforeEnter){const F={params:p,query:v,path:a,meta:m.meta},W=await me(m.beforeEnter,F);if(W){t.navigate(W);return}}const g=`${r.path}::${JSON.stringify(u.params)}`;if(V!==g){const F=await Xe(k,b);if(F){t.navigate(F);return}const W=r.transition??t.transition;W&&e.hasChildNodes()&&await ge(e,W,"leave"),C(T);const z=Ve(r,c);if(z){const M={routes:r.children,mode:t.mode,base:t.base,transition:r.transition??t.transition,beforeEach:t.beforeEach,afterEach:t.afterEach,scrollBehavior:t.scrollBehavior,navigate:(G,I)=>t.navigate(G,I),replace:(G,I)=>t.replace(G,I),back:()=>t.back(),forward:()=>t.forward()};k=await P(r,z,b,c==="default"?r.layout:void 0,c==="default"?M:void 0),(A=k.enter)==null||A.call(k,D)}else e.innerHTML="";V=g,W&&await ge(e,W,"enter")}const w={params:{...u.params,...p},query:v,path:a,meta:m.meta??r.meta};(n=t.afterEach)==null||n.call(t,w,D);const L=(h=t.scrollBehavior)==null?void 0:h.call(t,w,D);L&&window.scrollTo(L.x??0,L.y??0),D=w,T=r,H();return}}}const s=Ge(r.path,a);if(s!==null){V=null;const u={params:s,query:v,path:a,meta:r.meta};if(r.redirect){const w=typeof r.redirect=="function"?r.redirect(u):r.redirect;t.navigate(w);return}if(t.beforeEach){const w=await me(t.beforeEach,u);if(w){t.navigate(w);return}}if(r.beforeEnter){const w=await me(r.beforeEnter,u);if(w){t.navigate(w);return}}const m=await Xe(k,u);if(m){t.navigate(m);return}const p=r.transition??t.transition;p&&e.hasChildNodes()&&await ge(e,p,"leave"),C(T);const b=Ve(r,c);if(b){const w=u.path;if(r.keepAlive&&R.has(w)){const L=R.get(w);e.appendChild(L.fragment),k=L.activation,(f=k.activate)==null||f.call(k),R.delete(w)}else{const L=D;k=await P(r,b,u,c==="default"?r.layout:void 0),($=k.enter)==null||$.call(k,L)}}else e.innerHTML="",k=null;p&&await ge(e,p,"enter"),(O=t.afterEach)==null||O.call(t,u,D);const g=(i=t.scrollBehavior)==null?void 0:i.call(t,u,D);g&&window.scrollTo(g.x??0,g.y??0),D=u,T=r,H();return}}V=null,C(T),T=null,H()},d=t.mode==="history"?"popstate":"hashchange";return window.addEventListener(d,_),_(),()=>{window.removeEventListener(d,_),k==null||k.destroy(),k=null,R.forEach(({activation:a})=>a.destroy()),R.clear()}}function Ft(){if(typeof window>"u")return null;if(window.__COURVUX_DEVTOOLS__)return window.__COURVUX_DEVTOOLS__;const e=new Map,t={instances:[],stores:[],on(o,c){return e.has(o)||e.set(o,new Set),e.get(o).add(c),()=>{var l;return(l=e.get(o))==null?void 0:l.delete(c)}},_emit(o,c){var l;(l=e.get(o))==null||l.forEach(y=>{try{y(c)}catch{}})},_registerInstance(o){this.instances.push(o),this._emit("mount",o)},_unregisterInstance(o){const c=this.instances.findIndex(l=>l.id===o);if(c!==-1){const l=this.instances[c];this.instances.splice(c,1),this._emit("destroy",l)}},_registerStore(o){this.stores.push(o),o.subscribe(()=>this._emit("store-update",o))}};return window.__COURVUX_DEVTOOLS__=t,t}var Ot=0;function Lt(){return++Ot}var jt=`
...
`;function Pt(){if(document.getElementById("cvd-styles"))return;const e=document.createElement("style");e.id="cvd-styles",e.textContent=jt,document.head.appendChild(e)}function be(e){return e===null?"null":e===void 0?"undefined":typeof e=="string"?`"${e}"`:typeof e=="object"?JSON.stringify(e):String(e)}function Ke(e){try{return JSON.parse(e)}catch{return e}}function Dt(e){if(typeof document>"u")return;Pt();const t=document.createElement("div");t.id="cvd",document.body.appendChild(t);let o=!1,c="components",l=new Set;const y=document.createElement("div");y.id="cvd-badge",y.innerHTML='<span class="cvd-badge-dot"></span>COURVUX',t.appendChild(y);const E=document.createElement("div");E.id="cvd-panel",E.style.display="none",t.appendChild(E),E.innerHTML=`
        ...
        <div id="cvd-body"></div>
    `;const x=E.querySelector("#cvd-body");y.addEventListener("click",()=>{o=!0,y.style.display="none",E.style.display="flex",R()}),E.querySelector("#cvd-close").addEventListener("click",()=>{o=!1,E.style.display="none",y.style.display=""}),E.querySelectorAll(".cvd-tab").forEach(C=>{C.addEventListener("click",()=>{c=C.dataset.tab,E.querySelectorAll(".cvd-tab").forEach(P=>P.classList.remove("active")),C.classList.add("active"),R()})});const D=E.querySelector("#cvd-head");let T=!1,k=0,V=0;D.addEventListener("mousedown",C=>{C.target.closest("button")||(T=!0,k=C.clientX-t.getBoundingClientRect().left,V=C.clientY-t.getBoundingClientRect().top)}),document.addEventListener("mousemove",C=>{T&&(t.style.right="auto",t.style.bottom="auto",t.style.left=`${C.clientX-k}px`,t.style.top=`${C.clientY-V}px`)}),document.addEventListener("mouseup",()=>{T=!1});function j(){const C=e.instances;if(!C.length){x.innerHTML='<div class="cvd-empty">No hay componentes montados</div>';return}x.innerHTML=C.map(P=>{const _=P.getState(),d=Object.keys(_);return`
            ...
            `}).join(""),x.querySelectorAll(".cvd-inst-head").forEach(P=>{P.addEventListener("click",()=>{const _=P.closest(".cvd-inst"),d=parseInt(_.dataset.id);l.has(d)?l.delete(d):l.add(d),_.classList.toggle("open")})}),x.querySelectorAll(".cvd-val").forEach(P=>{P.addEventListener("click",_=>{_.stopPropagation();const d=P;if(d.querySelector("input"))return;const a=parseInt(d.dataset.inst),v=d.dataset.key,S=e.instances.find(f=>f.id===a);if(!S)return;const A=be(S.getState()[v]);d.classList.add("editing"),d.innerHTML=`<input class="cvd-edit" value='${A.replace(/'/g,"&#39;")}'>`;const n=d.querySelector("input");n.focus(),n.select();const h=()=>{S.setState(v,Ke(n.value)),d.classList.remove("editing")};n.addEventListener("blur",h),n.addEventListener("keydown",f=>{f.key==="Enter"&&(f.preventDefault(),h()),f.key==="Escape"&&(d.classList.remove("editing"),R())})})})}function H(){if(!e.stores.length){x.innerHTML='<div class="cvd-empty">No hay store registrado</div>';return}x.innerHTML=e.stores.map((C,P)=>{const _=C.getState(),d=Object.keys(_);return`
            ...
            `}).join(""),x.querySelectorAll(".cvd-inst-head").forEach(C=>{C.addEventListener("click",()=>C.closest(".cvd-inst").classList.toggle("open"))}),x.querySelectorAll("[data-store][data-key]").forEach(C=>{C.addEventListener("click",P=>{P.stopPropagation();const _=C;if(_.querySelector("input"))return;const d=parseInt(_.dataset.store),a=_.dataset.key,v=e.stores[d];if(!v)return;const S=be(v.getState()[a]);_.classList.add("editing"),_.innerHTML=`<input class="cvd-edit" value='${S.replace(/'/g,"&#39;")}'>`;const A=_.querySelector("input");A.focus(),A.select();const n=()=>{v.setState(a,Ke(A.value)),_.classList.remove("editing")};A.addEventListener("blur",n),A.addEventListener("keydown",h=>{h.key==="Enter"&&(h.preventDefault(),n()),h.key==="Escape"&&(_.classList.remove("editing"),R())})})})}function R(){o&&(c==="components"?j():H())}e.on("mount",()=>R()),e.on("update",()=>R()),e.on("destroy",()=>R()),e.on("store-update",()=>R())}var Mt="__COURVUX_HEAD_COLLECTOR__";function It(){return globalThis[Mt]??null}var Rt=(e,t)=>{Object.entries(t).forEach(([o,c])=>{o!=="innerHTML"&&(c==null||c===!1||e.setAttribute(o,c===!0?"":String(c)))})},zt=e=>{const t={};return Array.from(e.attributes).forEach(o=>{t[o.name]=o.value}),t},Ht=(e,t)=>{Array.from(e.attributes).forEach(o=>e.removeAttribute(o.name)),Object.entries(t).forEach(([o,c])=>e.setAttribute(o,c))},Qe=(e,t,o,c)=>{let l=o?document.head.querySelector(o):null;l?(c.push({el:l,prevAttrs:zt(l),created:!1}),Array.from(l.attributes).forEach(y=>l.removeAttribute(y.name))):(l=document.createElement(e),document.head.appendChild(l),c.push({el:l,created:!0})),Rt(l,t)};function Nt(e){var E,x,D;const t=It();if(t!==null)return t.push(e),()=>{};if(typeof document>"u")return()=>{};const o=[];let c;const l={},y={};if(e.title!==void 0){c=document.title;const T=e.titleTemplate,k=typeof T=="function"?T(e.title):typeof T=="string"?T.replace("%s",e.title):e.title;document.title=k}return(E=e.meta)==null||E.forEach(T=>{Qe("meta",T,T.name?`meta[name="${CSS.escape(T.name)}"]`:T.property?`meta[property="${CSS.escape(T.property)}"]`:T["http-equiv"]?`meta[http-equiv="${CSS.escape(T["http-equiv"])}"]`:null,o)}),(x=e.link)==null||x.forEach(T=>{Qe("link",T,T.rel==="canonical"?'link[rel="canonical"]':T.rel&&T.href?`link[rel="${CSS.escape(T.rel)}"][href="${CSS.escape(T.href)}"]`:null,o)}),(D=e.script)==null||D.forEach(T=>{const k=document.createElement("script");Object.entries(T).forEach(([V,j])=>{V==="innerHTML"?k.textContent=String(j):j!=null&&j!==!1&&k.setAttribute(V,j===!0?"":String(j))}),document.head.appendChild(k),o.push({el:k,created:!0})}),e.htmlAttrs&&Object.entries(e.htmlAttrs).forEach(([T,k])=>{l[T]=document.documentElement.getAttribute(T),document.documentElement.setAttribute(T,k)}),e.bodyAttrs&&Object.entries(e.bodyAttrs).forEach(([T,k])=>{y[T]=document.body.getAttribute(T),document.body.setAttribute(T,k)}),function(){c!==void 0&&(document.title=c),o.forEach(({el:T,prevAttrs:k,created:V})=>{var j;V?(j=T.parentNode)==null||j.removeChild(T):k&&Ht(T,k)}),Object.entries(l).forEach(([T,k])=>{k===null?document.documentElement.removeAttribute(T):document.documentElement.setAttribute(T,k)}),Object.entries(y).forEach(([T,k])=>{k===null?document.body.removeAttribute(T):document.body.setAttribute(T,k)})}}var Fe="data-courvux-ssr",Wt=e=>e?Promise.resolve().then(e):Promise.resolve();function et(e,t){const o=e.trim();if(o.startsWith("{")){const c=o.replace(/[{}]/g,"").split(",").map(l=>l.trim()).filter(Boolean);return Object.fromEntries(c.map(l=>[l,t[l]]))}return{[o]:t}}var ye=e=>{if(e===null||typeof e!="object")return e;try{return structuredClone(e)}catch{return e}};async function ne(e,t,o){var S,A,n;const c={},{subscribe:l,createReactiveState:y,registerSetInterceptor:E,notifyAll:x}=Pe();let D;if(typeof t.data=="function"?(t.loadingTemplate&&(e.innerHTML=t.loadingTemplate),D=await t.data()):D=t.data??{},t.templateUrl){const h=o.baseUrl?new URL(t.templateUrl,o.baseUrl).href:t.templateUrl,f=await fetch(h);if(!f.ok)throw new Error(`Failed to load template: ${h} (${f.status})`);e.innerHTML=await f.text()}else t.template&&(e.innerHTML=t.template);e.removeAttribute(Fe),(S=e.querySelector(`[${Fe}]`))==null||S.removeAttribute(Fe);const T={};if(t.inject&&o.provided){const h=Array.isArray(t.inject)?Object.fromEntries(t.inject.map(f=>[f,f])):t.inject;Object.entries(h).forEach(([f,$])=>{o.provided&&$ in o.provided&&(T[f]=o.provided[$])})}const k=y({...o.globalProperties??{},...D,...T,...t.methods,$refs:c,$el:e,...o.slots?{$slots:Object.fromEntries(Object.keys(o.slots).map(h=>[h,!0]))}:{},...o.store?{$store:o.store}:{},...o.currentRoute?{$route:o.currentRoute}:{},...o.router?{$router:o.router}:{}});k.$watch=(h,f,$)=>{const O=($==null?void 0:$.deep)??!1,i=($==null?void 0:$.immediate)??!1;let r=O?ye(k[h]):k[h];const s=l(h,()=>{const u=k[h];f.call(k,u,r),r=O?ye(u):u});return i&&f.call(k,k[h],void 0),s},k.$batch=yt,k.$nextTick=h=>Wt(h),k.$dispatch=(h,f,$)=>{e.dispatchEvent(new CustomEvent(h,{bubbles:!0,composed:!0,...$??{},detail:f}))},o.magics&&Object.entries(o.magics).forEach(([h,f])=>{k[h]=f(k)}),k.$forceUpdate=()=>x();const V=[];k.$watchEffect=h=>{let f=[];const $=()=>{f.forEach(s=>s()),f=[];const i=Ie(()=>{try{h()}catch{}}),r=new Map;for(const{sub:s,key:u}of i)r.has(s)||r.set(s,new Set),!r.get(s).has(u)&&(r.get(s).add(u),f.push(s(u,$)))};$();const O=()=>{f.forEach(r=>r()),f=[];const i=V.indexOf(O);i>-1&&V.splice(i,1)};return V.push(O),O};const j=[];t.computed&&Object.entries(t.computed).forEach(([h,f])=>{const $=typeof f=="function"?f:f.get,O=typeof f!="function"?f.set:void 0;let i=[];const r=()=>{i.forEach(p=>p()),i=[];let s;const u=Ie(()=>{try{s=$.call(k)}catch(p){(t.debug??o.debug)&&console.warn("[courvux] computed error:",p)}});k[h]=s;const m=new Map;for(const{sub:p,key:b}of u)m.has(p)||m.set(p,new Set),!m.get(p).has(b)&&(m.get(p).add(b),i.push(p(b,r)))};r(),j.push(()=>i.forEach(s=>s())),O&&E(h,s=>O.call(k,s))});const H=[];t.watch&&Object.entries(t.watch).forEach(([h,f])=>{const $=typeof f=="object"&&f!==null&&"handler"in f,O=$?f.handler:f,i=$?f.immediate??!1:!1,r=$?f.deep??!1:!1;let s=r?ye(k[h]):k[h];const u=l(h,()=>{const m=k[h];O.call(k,m,s),s=r?ye(m):m});H.push(u),i&&O.call(k,k[h],void 0)});const R={...o.provided??{}};if(t.provide){const h=typeof t.provide=="function"?t.provide.call(k):t.provide;Object.assign(R,h)}const C={...o,provided:R,components:{...o.components,...t.components}};C.mountElement=Ae(C),C.createChildScope=(h,f)=>{const $=new Set(Object.keys(h)),O=new Set(Object.keys(f)),{subscribe:i,createReactiveState:r}=Pe(),s=r(h);let u;return u=new Proxy({},{get(m,p){return typeof p!="string"?k[p]:$.has(p)?s[p]:O.has(p)?f[p].bind(u):k[p]},set(m,p,b){return typeof p!="string"?!1:$.has(p)?(s[p]=b,!0):(k[p]=b,!0)},has(m,p){return $.has(p)||O.has(p)||p in k},ownKeys(){return[...$,...O,...Object.keys(k)]},getOwnPropertyDescriptor(m,p){return $.has(p)||O.has(p)||p in k?{configurable:!0,enumerable:!0,writable:!0}:void 0}}),{state:u,subscribe:(m,p)=>$.has(m)?i(m,p):d(m,p),cleanup:()=>{}}},C.mountDynamic=async(h,f,$,O,i)=>{let r=null,s=null;const u=$.getAttribute("loading-template")??"",m=async()=>{var z,M,G;s==null||s(),s=null,r!=null&&r.parentNode&&(r.parentNode.removeChild(r),r=null);const p=N(f,O);if(!p)return;let b;if(typeof p=="function"){if(u){const I=document.createElement("div");I.innerHTML=u,(z=h.parentNode)==null||z.insertBefore(I,h.nextSibling),r=I}b=(await p()).default,r!=null&&r.parentNode&&(r.parentNode.removeChild(r),r=null)}else typeof p=="string"?b=(M=C.components)==null?void 0:M[p]:p&&typeof p=="object"&&(b=p);if(!b)return;const g=document.createElement("div");Array.from($.attributes).forEach(I=>g.setAttribute(I.name,I.value)),g.innerHTML=$.innerHTML;const w={},L={};Array.from($.attributes).forEach(I=>{if(I.name.startsWith(":"))w[I.name.slice(1)]=N(I.value,O);else if(I.name.startsWith("@")||I.name.startsWith("cv:on:")){const B=I.value,Q=I.name.startsWith("@")?I.name.slice(1):I.name.slice(6);L[Q]=(...q)=>{typeof O[B]=="function"&&O[B].call(O,...q)}}});const F={...b,data:{...b.data,...w},methods:{...b.methods,$emit(I,...B){var Q;pt(b,I,B),(Q=L[I])==null||Q.call(L,...B)}}},W={...C,components:{...C.components,...b.components}};W.mountElement=Ae(W),s=(await ne(g,F,W)).destroy,(G=h.parentNode)==null||G.insertBefore(g,h.nextSibling),r=g};K(f,i,m),await m()};const P=[];k.$addCleanup=h=>{P.push(h)};let _=!1;const d=(h,f)=>!t.onBeforeUpdate&&!t.onUpdated?l(h,f):l(h,()=>{var $;_||(_=!0,($=t.onBeforeUpdate)==null||$.call(k),Promise.resolve().then(()=>{var O;_=!1,(O=t.onUpdated)==null||O.call(k)})),f()});try{(A=t.onBeforeMount)==null||A.call(k),await oe(e,k,{subscribe:d,refs:c,...C,registerCleanup:h=>P.push(h)}),e.removeAttribute("cv-cloak"),(n=t.onMount)==null||n.call(k)}catch(h){if(t.onError)e.removeAttribute("cv-cloak"),t.onError.call(k,h);else if(o.errorHandler)e.removeAttribute("cv-cloak"),o.errorHandler(h,k,t.name??e.tagName.toLowerCase());else throw h}const a=typeof window<"u"?window.__COURVUX_DEVTOOLS__:void 0,v=a?Lt():0;if(a){const h=k,f=new Set,$={id:v,name:t.name??e.tagName.toLowerCase(),el:e,getState:()=>{const O={};for(const i of Object.keys(h))if(!(i.startsWith("$")||typeof h[i]=="function"))try{O[i]=h[i]}catch{}return O},setState:(O,i)=>{h[O]=i},subscribe:O=>(f.add(O),()=>f.delete(O)),children:[]};Object.keys(h).filter(O=>!O.startsWith("$")&&typeof h[O]!="function").forEach(O=>{l(O,()=>{a._emit("update",$),f.forEach(i=>i())})}),a._registerInstance($),P.push(()=>a._unregisterInstance(v))}return{state:k,destroy:()=>{var h,f;(h=t.onBeforeUnmount)==null||h.call(k),j.forEach($=>$()),H.forEach($=>$()),V.forEach($=>$()),P.forEach($=>$()),(f=t.onDestroy)==null||f.call(k)},activate:()=>{var h;(h=t.onActivated)==null||h.call(k)},deactivate:()=>{var h;(h=t.onDeactivated)==null||h.call(k)},beforeLeave:t.onBeforeRouteLeave?(h,f)=>t.onBeforeRouteLeave.call(k,h,f):void 0,enter:t.onBeforeRouteEnter?h=>t.onBeforeRouteEnter.call(k,h):void 0}}function pt(e,t,o){if(!e.emits||Array.isArray(e.emits))return;const c=e.emits[t];typeof c=="function"&&!c(...o)&&console.warn(`[courvux] emit "${t}": validator returned false`)}function Ae(e){return async(t,o,c,l)=>{const y=e.components[o],E=t.getAttribute("cv-ref");E&&t.removeAttribute("cv-ref");const x={},D=[],T={};Array.from(t.attributes).filter(_=>_.name==="cv-model"||_.name.startsWith("cv-model.")||_.name.startsWith("cv-model:")).forEach(_=>{t.removeAttribute(_.name);const d=_.value,a=_.name.indexOf(":"),v=a>=0?_.name.slice(a+1).split(".")[0]:"modelValue",S=v==="modelValue"?"update:modelValue":`update:${v}`;x[v]=Te(N(d,c)),D.push({propName:v,expr:d}),T[S]=A=>{le(d,c,A)}});const k={};Array.from(t.attributes).forEach(_=>{const d=_.name.startsWith(":"),a=_.name.startsWith("@")||_.name.startsWith("cv:on:"),v=_.name==="cv-model"||_.name.startsWith("cv-model.")||_.name.startsWith("cv-model:"),S=_.name.startsWith("v-slot"),A=_.name==="slot";!d&&!a&&!v&&!S&&!A&&(k[_.name]=_.value)}),y.inheritAttrs===!1&&Object.keys(k).forEach(_=>t.removeAttribute(_)),Array.from(t.attributes).forEach(_=>{if(_.name.startsWith(":")){const d=_.name.slice(1),a=_.value;x[d]=Te(N(a,c)),D.push({propName:d,expr:a})}else if(_.name.startsWith("@")||_.name.startsWith("cv:on:")){const d=_.name.startsWith("@")?_.name.slice(1):_.name.slice(6),a=_.value;T[d]=(...v)=>{typeof c[a]=="function"&&c[a].call(c,...v)}}});const V=t.getAttribute("v-slot")??t.getAttribute("v-slot:default");V&&(t.removeAttribute("v-slot"),t.removeAttribute("v-slot:default"));const j=new Map,H=[];Array.from(t.childNodes).forEach(_=>{const d=_.nodeType===1?_.getAttribute("slot"):null;if(d){if(!j.has(d)){const a=t.getAttribute(`v-slot:${d}`)??null;a&&t.removeAttribute(`v-slot:${d}`),j.set(d,{nodes:[],vSlot:a})}j.get(d).nodes.push(_.cloneNode(!0))}else H.push(_.cloneNode(!0))});const R={};H.some(_=>{var d;return _.nodeType===1||_.nodeType===3&&(((d=_.textContent)==null?void 0:d.trim())??"")!==""})&&(R.default=async _=>{const d=V?{...c,...et(V,_)}:c,a=document.createDocumentFragment();return H.forEach(v=>a.appendChild(v.cloneNode(!0))),await oe(a,d,l),Array.from(a.childNodes)});for(const[_,{nodes:d,vSlot:a}]of j)R[_]=async v=>{const S=a?{...c,...et(a,v)}:c,A=document.createDocumentFragment();return d.forEach(n=>A.appendChild(n.cloneNode(!0))),await oe(A,S,l),Array.from(A.childNodes)};const C={...e,components:{...e.components,...y.components},slots:R};C.mountElement=Ae(C);const{state:P}=await ne(t,{...y,data:{...y.data,...x,$attrs:k,$parent:c},methods:{...y.methods,$emit(_,...d){var a;pt(y,_,d),(a=T[_])==null||a.call(T,...d)}}},C);P&&(D.forEach(({propName:_,expr:d})=>{ue(d,{...l,subscribe:l.subscribe},()=>{P[_]=Te(N(d,c))})}),E&&l.refs&&(l.refs[E]=P))}}function Ut(e){wt();const t=typeof window<"u"?Ft():void 0,o=[],c={...e.directives},l={...e.components??{}},y=[],E=new Map,x={},D=new Map;if(e.debug&&t&&Dt(t),t&&e.store){const j=e.store,H=Object.keys(j).filter(R=>typeof j[R]!="function");t._registerStore({getState(){const R={};return H.forEach(C=>{try{R[C]=j[C]}catch{}}),R},setState(R,C){j[R]=C},subscribe(R){const C=H.map(P=>{try{return ve(j,P,R)}catch{return()=>{}}});return()=>C.forEach(P=>P())}})}const T={router:e.router,use(j){return o.includes(j)||(o.push(j),j.install(T)),T},directive(j,H){return c[j]=H,T},component(j,H){return l[j]=H,T},provide(j,H){return typeof j=="string"?x[j]=H:Object.assign(x,j),T},magic(j,H){return D.set(`$${j}`,H),T},mount:async j=>(await V(j),T),mountAll:async(j="[data-courvux]")=>{const H=Array.from(document.querySelectorAll(j));return await Promise.all(H.map(R=>k(R))),T},mountEl:async j=>k(j),unmount(j){if(!j)y.forEach(H=>H()),y.length=0,E.clear();else{const H=document.querySelector(j);if(H){const R=E.get(H);if(R){R(),E.delete(H);const C=y.indexOf(R);C>-1&&y.splice(C,1)}}}return T},destroy(){y.forEach(j=>j()),y.length=0,E.clear()}},k=async j=>{const H=new URL(".",document.baseURI).href,R={components:l,router:e.router,store:e.store,directives:c,baseUrl:H,provided:{...x},errorHandler:e.errorHandler,globalProperties:e.globalProperties,magics:D.size?Object.fromEntries(D):void 0};if(R.mountElement=Ae(R),e.router){const P=e.router;R.mountRouterView=async(_,d)=>{await new Promise(a=>{Je(_,P,async(v,S,A,n,h)=>{const f={...R,currentRoute:A};if(h){let $=null;const O={...f,mountRouterView:async(i,r)=>{$=Je(i,h,async(s,u,m,p)=>{const b={...f,currentRoute:m};if(p){let g=null;const w={...b,mountRouterView:async(F,W)=>{g=await ne(F,u,b)}},{destroy:L}=await ne(s,{template:p},w);return{destroy:()=>{g==null||g.destroy(),L()},activate:()=>g==null?void 0:g.activate(),deactivate:()=>g==null?void 0:g.deactivate()}}else return await ne(s,u,b)},r)}};if(n){let i=null;const r={...O,mountRouterView:async(u,m)=>{i=await ne(u,S,O)}},{destroy:s}=await ne(v,{template:n},r);return{destroy:()=>{$==null||$(),i==null||i.destroy(),s()},activate:()=>i==null?void 0:i.activate(),deactivate:()=>i==null?void 0:i.deactivate()}}else{const i=await ne(v,S,O);return{destroy:()=>{$==null||$(),i.destroy()},activate:()=>i.activate(),deactivate:()=>i.deactivate()}}}else if(n){let $=null;const O={...f,mountRouterView:async(r,s)=>{$=await ne(r,S,f)}},{destroy:i}=await ne(v,{template:n},O);return{destroy:()=>{$==null||$.destroy(),i()},activate:()=>$==null?void 0:$.activate(),deactivate:()=>$==null?void 0:$.deactivate()}}else return await ne(v,S,f)},d,a)})}}const C=await ne(j,e,R);return y.push(C.destroy),E.set(j,C.destroy),C.state},V=async j=>{const H=document.querySelector(j);if(H)return k(H)};return T}var tt=typeof globalThis<"u"?globalThis:typeof window<"u"?window:typeof global<"u"?global:typeof self<"u"?self:{};function Bt(e){return e&&e.__esModule&&Object.prototype.hasOwnProperty.call(e,"default")?e.default:e}var Oe={exports:{}},ot;function qt(){return ot||(ot=1,(function(e){var t=typeof window<"u"?window:typeof WorkerGlobalScope<"u"&&self instanceof WorkerGlobalScope?self:{};/**
...
pnpm add github:vanjexdev/courvux#v0.4.1`,s2:`<script type="importmap">
{
  "imports": {
...
<editor cv-model:title="docTitle" cv-model:body="docBody" />`},template:`
    ...
    `,onMount(){Z({title:"Components",description:"Define, register, and compose Courvux components with props, slots, emits, and scoped slots.",slug:"/components"})}},ro={data:{s_computed:`{
    data: { price: 10, qty: 3 },
    computed: {
...
provide() {
    ...
    `,onMount(){Z({title:"Reactivity",description:"Proxy-based reactive state, computed properties, watchers, and refs in Courvux.",slug:"/reactivity"})}},ao={data:{s1:`{
    data: { ticks: 0 },
...
<button cv-show="todos.some(t => t.done)" @click="clearCompleted()">Clear completed</button>`;function yo(){try{return JSON.parse(localStorage.getItem(mt))||[]}catch{return[]}}function xo(e){localStorage.setItem(mt,JSON.stringify(e))}const wo={data:{todos:yo(),newTodo:"",filter:"all",editingId:null,editText:"",_nextId:Date.now(),srcJs:go,srcHtml:bo,srcTab:"js"},computed:{filteredTodos(){return this.filter==="active"?this.todos.filter(e=>!e.done):this.filter==="completed"?this.todos.filter(e=>e.done):this.todos},remaining(){return this.todos.filter(e=>!e.done).length},allDone(){return this.todos.length>0&&this.todos.every(e=>e.done)}},watch:{todos:{deep:!0,handler(e){xo(e)}}},methods:{add(){const e=this.newTodo.trim();e&&(this.todos=[...this.todos,{id:this._nextId++,text:e,done:!1}],this.newTodo="")},toggle(e){this.todos=this.todos.map(t=>t.id===e?{...t,done:!t.done}:t)},remove(e){this.todos=this.todos.filter(t=>t.id!==e)},clearCompleted(){this.todos=this.todos.filter(e=>!e.done)},toggleAll(){const e=this.allDone;this.todos=this.todos.map(t=>({...t,done:!e}))},startEdit(e){this.editingId=e.id,this.editText=e.text,this.$nextTick(()=>{var t;return(t=this.$refs["edit_"+e.id])==null?void 0:t.focus()})},commitEdit(e){const t=this.editText.trim();t?this.todos=this.todos.map(o=>o.id===e?{...o,text:t}:o):this.remove(e),this.editingId=null,this.editText=""},cancelEdit(){this.editingId=null,this.editText=""},setFilter(e){this.filter=e}},template:`
    ...
    `,onMount(){Z({title:"Demo — TODO App",description:"Interactive TODO app built with Courvux. Live demo with full source code (JS + HTML).",slug:"/demo"})}},ko=[{path:"/",component:Qt},{path:"/installation",component:eo},{path:"/quick-start",component:to},{path:"/template",component:oo},{path:"/components",component:no},{path:"/reactivity",component:ro},{path:"/lifecycle",component:ao},{path:"/composables",component:io},{path:"/event-bus",component:so},{path:"/router",component:co},{path:"/store",component:lo},{path:"/head",component:uo},{path:"/ssg",component:po},{path:"/devtools",component:mo},{path:"/testing",component:ho},{path:"/pwa",component:fo},{path:"/advanced",component:vo},{path:"/demo",component:wo}],Ao={methods:{goBack(){window.history.back()}},template:`
