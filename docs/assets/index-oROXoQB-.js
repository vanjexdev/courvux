...
`)}const io={data:{lang:"js",code:"",label:"",copied:!1},template:`
    ...
    `,exprs:{"label || langLabel":(e=>e.label||e.langLabel),"copied ? '✓ Copied' : 'Copy'":(e=>e.copied?"â Copied":"Copy"),"copy()":(e=>e.copy()),"'language-' + lang":(e=>"language-"+e.lang)},computed:{langLabel(){return so[this.lang]||this.lang.toUpperCase()}},methods:{copy(){var e;(e=navigator.clipboard)==null||e.writeText(this._cleanCode).then(()=>{this.copied=!0,setTimeout(()=>{this.copied=!1},1800)})}},onMount(){const e=this.$refs.el;e&&(this._cleanCode=ao(this.code),e.textContent=this._cleanCode,no.highlightElement(e))}},ve="Courvux",co="https://vanjexdev.github.io/courvux";function J({title:e,description:t,slug:o="/"}){const i=e?`${e} — ${ve}`:`${ve} — Lightweight reactive UI framework`,s=co+o;return Zt({title:e??`${ve} — Lightweight reactive UI framework`,titleTemplate:e?`%s — ${ve}`:void 0,meta:[{name:"description",content:t},{property:"og:title",content:i},{property:"og:description",content:t},{property:"og:type",content:"website"},{property:"og:url",content:s},{property:"og:site_name",content:ve},{name:"twitter:card",content:"summary_large_image"},{name:"twitter:title",content:i},{name:"twitter:description",content:t}],link:[{rel:"canonical",href:s}]})}const lo={data:{install:`# From GitHub — pin a tag for stable installs
...
createApp({
    ...
    `,exprs:{"'bash'":(e=>"bash"),install:(e=>e.install),"'terminal'":(e=>"terminal"),"'js'":(e=>"js"),counter:(e=>e.counter),"'main.js'":(e=>"main.js")},onMount(){J({description:"Lightweight reactive UI framework for the browser. No virtual DOM. Proxy-based reactivity. ~20 KB gzip with everything (router, store, devtools, composables, useHead, SSR).",slug:"/"})}},po={data:{s1:`# Latest commit on main (rolling)
...
<\/script>`},onMount(){J({title:"Installation",description:"Install Courvux from GitHub or via import map / CDN. Vite plugin for templateUrl inlining and jsDelivr CDN setup.",slug:"/installation"});const e=this.$refs.pen;if(!e)return;const t=document.createElement("iframe");t.src="https://codepen.io/jesusuzcategui-the-bold/embed/VYmZNgZ?default-tab=html%2Cresult&theme-id=dark",t.height="420",t.style.cssText="width:100%; border:0; border-radius:8px; display:block; margin-bottom:1rem;",t.scrolling="no",t.setAttribute("frameborder","no"),t.setAttribute("allowtransparency","true"),t.allowFullscreen=!0,t.title="Courvux CDN demo on CodePen",e.replaceWith(t)},template:`
    ...
    `,exprs:{"'html'":(e=>"html"),sCdn:(e=>e.sCdn),"'index.html'":(e=>"index.html"),sCdnMap:(e=>e.sCdnMap),"'bash'":(e=>"bash"),s1:(e=>e.s1),"'terminal'":(e=>"terminal"),s2:(e=>e.s2),"'js'":(e=>"js"),s3:(e=>e.s3),"'main.js'":(e=>"main.js"),s4:(e=>e.s4),"'vite.config.js'":(e=>"vite.config.js"),"'json'":(e=>"json"),s5:(e=>e.s5),"'tsconfig.json'":(e=>"tsconfig.json")}},uo={data:{s1:`import { createApp } from 'courvux';
...
createApp({ store, router, template: '<router-view />' }).mount('#app');`},template:`
    ...
    `,exprs:{"'js'":(e=>"js"),s1:(e=>e.s1),"'main.js'":(e=>"main.js"),s2:(e=>e.s2),s3:(e=>e.s3),s4:(e=>e.s4)},onMount(){J({title:"Quick Start",description:"Build your first reactive Courvux app — counter example, methods, computed properties.",slug:"/quick-start"})}},mo={data:{s_interp:`<!-- Text interpolation -->
<p>{{ count }}</p>
<p>{{ price * qty }}</p>
<p>{{ active ? 'On' : 'Off' }}</p>
...
<div id="app" cv-cloak></div>`},template:`
    ...
    `,exprs:{"'html'":(e=>"html"),s_interp:(e=>e.s_interp),s_bind:(e=>e.s_bind),s_events:(e=>e.s_events),s_cvfor:(e=>e.s_cvfor),s_cvif:(e=>e.s_cvif),s_cvshow:(e=>e.s_cvshow),s_cvmodel:(e=>e.s_cvmodel),s_cvhtml:(e=>e.s_cvhtml),s_cvdata:(e=>e.s_cvdata),s_misc:(e=>e.s_misc)},onMount(){J({title:"Template Syntax",description:"Courvux directives and bindings: cv-if, cv-for, cv-model, cv-show, :class, :style, @event.",slug:"/template"})}},ho={data:{s_define:`createApp({
    components: {
        'user-card': {
...
}`,s3:`// Error boundary — catches errors from descendant onMount
{
    ...
    `,exprs:{"'js'":(e=>"js"),s1:(e=>e.s1),s2:(e=>e.s2),s3:(e=>e.s3)},onMount(){J({title:"Lifecycle Hooks",description:"onMount, onBeforeUnmount, onDestroy, error boundaries, and async data in Courvux.",slug:"/lifecycle"})}},vo={data:{s_storage:`import { cvStorage } from 'courvux';
...
bus.emit('toast:show', 'Saved again');  // handler does NOT run`},template:`
    ...
    `,exprs:{"'js'":(e=>"js"),s_basic:(e=>e.s_basic),"'ts'":(e=>"ts"),s_typed:(e=>e.s_typed),s_once:(e=>e.s_once),s_provide:(e=>e.s_provide),s_inject:(e=>e.s_inject)},onMount(){J({title:"Event Bus",description:"Typed cross-component event bus in Courvux: on, off, emit, once, clear, and provide/inject patterns.",slug:"/event-bus"})}},yo={data:{s_setup:`import { createApp, createRouter } from 'courvux';
...
methods: {
    ...
    `,exprs:{"'js'":(e=>"js"),s_setup:(e=>e.s_setup),s_route:(e=>e.s_route),s_params:(e=>e.s_params),s_lazy:(e=>e.s_lazy),s_navigate:(e=>e.s_navigate),s_nested:(e=>e.s_nested)},onMount(){J({title:"Router",description:"SPA routing in Courvux: dynamic params, nested routes, navigation guards, transitions.",slug:"/router"})}},xo={data:{s1:`import { createStore } from 'courvux';
...
{ inject: { localTheme: 'theme', endpoint: 'apiUrl' } }`},template:`
    ...
    `,exprs:{"'js'":(e=>"js"),s1:(e=>e.s1),"'html'":(e=>"html"),s2:(e=>e.s2),s3:(e=>e.s3),s4:(e=>e.s4),s5:(e=>e.s5),s6:(e=>e.s6)},onMount(){J({title:"Store",description:"Global reactive state in Courvux with createStore, modules, and namespaced actions.",slug:"/store"})}},wo={data:{s_basic:`import { useHead } from 'courvux';
...
// → embed headHtml in your shell, then html in the mount root`},template:`
    ...
    mode: 'history',
    base: '/courvux',
});`)},onMount(){J({title:"Static Site Generation",description:"Pre-render every route to static HTML at build time with the courvux/plugin/ssg Vite plugin.",slug:"/ssg"})}},So={data:{s_setup:`import { createApp, setupDevTools, mountDevOverlay } from 'courvux';
...
expect(w.text()).toBe('Alice');`,s_async:`it('waits for async data', async () => {
    ...
    `,exprs:{"'js'":(e=>"js"),s_config:(e=>e.s_config),s_basic:(e=>e.s_basic),s_state:(e=>e.s_state),s_global:(e=>e.s_global),s_async:(e=>e.s_async)},onMount(){J({title:"Testing",description:"Vitest-compatible test utility for Courvux. Mount components, drive state, query the DOM with happy-dom.",slug:"/testing"})}},Ao={data:{s_manifest:`{
  "name":             "My App",
  "short_name":       "MyApp",
...
\`\`\``},template:`
    ...
    `,exprs:{count:(e=>e.count),"'html'":(e=>"html"),s_csp:(e=>e.s_csp),"!flag":(e=>!e.flag),"save()":(e=>e.save()),"form.title":(e=>e.form.title),"'bash'":(e=>"bash"),s_storage:(e=>e.s_storage),"'text'":(e=>"text"),s_layout:(e=>e.s_layout),s_install:(e=>e.s_install)},onMount(){J({title:"Notepad — Tauri 2 demo",description:"Desktop notepad app showcasing Courvux + Tauri 2 + Tailwind 4 + the courvux-precompile plugin: Markdown editor with live preview, atomic per-note Markdown files, native folder picker, strict CSP without unsafe-eval.",slug:"/projects/notepad"})}},Ro=[{path:"/",component:lo},{path:"/installation",component:po},{path:"/quick-start",component:uo},{path:"/template",component:mo},{path:"/components",component:ho},{path:"/reactivity",component:fo},{path:"/lifecycle",component:go},{path:"/composables",component:vo},{path:"/event-bus",component:bo},{path:"/router",component:yo},{path:"/store",component:xo},{path:"/head",component:wo},{path:"/ssg",component:ko},{path:"/devtools",component:So},{path:"/testing",component:_o},{path:"/pwa",component:Ao},{path:"/csp",component:Co},{path:"/advanced",component:To},{path:"/design-decisions",component:$o},{path:"/faq",component:Eo},{path:"/migrating-from-vue",component:jo},{path:"/migrating-from-alpine",component:Po},{path:"/demo",component:Mo},{path:"/projects/notepad",component:Lo}],zo={methods:{goBack(){window.history.back()}},template:`
